---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for OpenHand deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, OpenHand cron scheduling for periodic checks, or version status checks on a machine running OpenHand (laptop, workstation, Pi, VPS).
---

# OpenHand Host Hardening

## Overview

Assess and harden the host running OpenHand, then align it to a user-defined risk tolerance without breaking access. Use OpenHand security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim OpenHand changes the host firewall, SSH, or OS updates; it does not.
...
5. OpenHand gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if OpenHand can access sensitive data.
...
### 2) Run OpenHand security audits (read-only)

As part of the default read-only checks, run `openhand security audit --deep`. Only offer alternatives if the user requests them:

1. `openhand security audit` (faster, non-probing)
2. `openhand security audit --json` (structured output)

Offer to apply OpenHand safe defaults (numbered):

1. `openhand security audit --fix`

Be explicit that `--fix` only tightens OpenHand defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check OpenHand version/update status (read-only)

As part of the default read-only checks, run `openhand update status`.
...
- Credential hygiene notes (location of OpenHand creds, prefer disk encryption)
...
- OpenHand security audit (re-run)
...
After OpenHand install or first hardening pass, run at least one baseline audit and version check:

- `openhand security audit`
- `openhand security audit --deep`
- `openhand update status`

Ongoing monitoring is recommended. Use the OpenHand cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `openhand cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `openhand update status`
...
Before creating, `openhand cron list` and match on exact `name`. If found, `openhand cron edit <id> ...`.
If not found, `openhand cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `openhand update status` (preferred for source checkouts and channels)
2. `npm view openhand version` (published npm version)

## OpenHand command accuracy

Use only supported commands and flags:

- `openhand security audit [--deep] [--fix] [--json]`
- `openhand status` / `openhand status --deep`
- `openhand health --json`
- `openhand update status`
- `openhand cron add|list|runs|run`

Do not invent CLI flags or imply OpenHand enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by OpenHand compaction:
