...
# Build and bundle OpenHand into a minimal .app we can open.
# Outputs to dist/OpenHand.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/OpenHand.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="OpenHand"
BUNDLE_ID="${BUNDLE_ID:-ai.openhand.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/openhand/openhand/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/OpenHand/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :OpenHandBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :OpenHandGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/OpenHand"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/OpenHand"
fi
chmod +x "$APP_ROOT/Contents/MacOS/OpenHand"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/OpenHand" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/OpenHand/Resources/OpenHand.icns" "$APP_ROOT/Contents/Resources/OpenHand.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/OpenHand/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OpenHandKit resources"
OPENHANDKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OpenHandKit_OpenHandKit.bundle"
if [ -d "$OPENHANDKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OpenHandKit_OpenHandKit.bundle"
  cp -R "$OPENHANDKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OpenHandKit_OpenHandKit.bundle"
else
  echo "WARN: OpenHandKit resource bundle not found at $OPENHANDKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running OpenHand"
killall -q OpenHand 2>/dev/null || true
