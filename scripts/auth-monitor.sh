...
#
# Suggested cron: */30 * * * * /home/admin/openhand/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send OpenHand notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., openhand-alerts)
...
STATE_FILE="$HOME/.openhand/auth-monitor-state"
...
send_notification() {
    ...
    # Send via OpenHand if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use openhand
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via OpenHand to $NOTIFY_PHONE..."
            openhand send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! OpenHand is down. Run: ssh l36 '~/openhand/scripts/mobile-reauth.sh'" "urgent"
    exit 1
