# OpenHand Installer for Windows (PowerShell)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🖐 OpenHand Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one OpenHand.$NC" -Level info
    Write-Host ""
}
...
function Install-OpenHandNpm {
    param([string]$Version = "latest")
    
    Write-Host "Installing OpenHand (openclaw@$Version)..." -Level info
    
    try {
        # Use -ExecutionPolicy Bypass to handle restricted execution policy
        npm install -g openclaw@$Version --no-fund --no-audit 2>&1
        Write-Host "OpenHand installed" -Level success
        return $true
    } catch {
...
function Install-OpenHandGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing OpenHand from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/Eythos-Foundation/OpenHand.git $RepoDir 2>&1
    } elseif ($Update) {
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install OpenHand from git to $GitDir" -Level info
        } else {
            Install-OpenHandGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install OpenHand via npm (tag: $Tag)" -Level info
        } else {
            if (!(Install-OpenHandNpm -Version $Tag)) {
                exit 1
            }
        }
    }
    ...
    Write-Host ""
    Write-Host "🖐 OpenHand installed successfully!" -Level success
}
