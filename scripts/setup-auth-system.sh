#!/bin/bash
# Setup OpenHand Auth Management System
...
echo "=== OpenHand Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'openhand-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  OpenHand message: Send warning via OpenHand itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/openhand-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/openhand-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/openhand-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now openhand-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status openhand-auth-monitor.timer"
echo ""
echo "Quick commands:"
