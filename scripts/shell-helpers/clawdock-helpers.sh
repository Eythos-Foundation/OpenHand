#!/usr/bin/env bash
# ClawDock - Docker helpers for OpenHand
# Inspired by Simon Willison's "Running OpenHand in Docker"
# https://til.simonwillison.net/llms/openhand-docker
#
# Installation:
...
# Common paths to check for OpenHand
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/openhand"
  "${HOME}/workspace/openhand"
  "${HOME}/projects/openhand"
  "${HOME}/dev/openhand"
  "${HOME}/code/openhand"
  "${HOME}/src/openhand"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo " Found OpenHand at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/openhand"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ OpenHand not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/openhand/openhand.git ~/openhand"
    echo "  cd ~/openhand && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^OPENHAND_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d openhand-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart openhand-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f openhand-gateway
}
...
clawdock-workspace() {
  cd ~/.openhand/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec openhand-gateway \
    bash -c 'echo "alias openhand=\"./openhand.mjs\"" > /tmp/.bashrc_openhand && bash --rcfile /tmp/.bashrc_openhand'
}
...
clawdock-cli() {
  _clawdock_compose run --rm openhand-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build openhand-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENHAND_GATEWAY_TOKEN=$token" openhand-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" openhand-gateway \
    bash -c './openhand.mjs config set gateway.remote.token "$TOKEN" && ./openhand.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec openhand-gateway \
    bash -c "./openhand.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart openhand-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm openhand-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec openhand-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'openhand config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN} ClawDock - Docker Helpers for OpenHand${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (openhand alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to openhand project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.openhand)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'openhand channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'openhand status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.openhand.ai${_CLR_RESET}"
  echo ""
}
