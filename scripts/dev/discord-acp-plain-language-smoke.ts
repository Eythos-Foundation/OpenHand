...
type DriverMode = "token" | "webhook" | "openhand";
...
function resolveStateDir(): string {
  const override = process.env.OPENHAND_STATE_DIR?.trim() || process.env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.OPENHAND_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".openhand");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver openhand] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) OpenHand spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|openhand> Driver transport mode (default: token)\n" +
    ...
    "  --openhand-bin <path>        OpenHand CLI binary for driver=openhand (default: openhand)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  OPENHAND_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  OPENHAND_DISCORD_SMOKE_DRIVER\n" +
    "  OPENHAND_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  OPENHAND_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  OPENHAND_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  OPENHAND_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  OPENHAND_DISCORD_SMOKE_AGENT\n" +
    "  OPENHAND_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  OPENHAND_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  OPENHAND_DISCORD_SMOKE_POLL_MS\n" +
    "  OPENHAND_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  OPENHAND_DISCORD_SMOKE_OPENHAND_BIN"
  );
}
...
function parseArgs(): Args {
  const channelId =
    resolveArg("--channel") ||
    process.env.OPENHAND_DISCORD_SMOKE_CHANNEL_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_CHANNEL_ID ||
    "";
  const driverModeRaw =
    resolveArg("--driver") ||
    process.env.OPENHAND_DISCORD_SMOKE_DRIVER ||
  ...
  const driverToken =
    resolveArg("--token") ||
    process.env.OPENHAND_DISCORD_SMOKE_DRIVER_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_DRIVER_TOKEN ||
    "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.OPENHAND_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.OPENHAND_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.CLAWDBOT_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.OPENHAND_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent") ||
    process.env.OPENHAND_DISCORD_SMOKE_AGENT ||
    process.env.CLAWDBOT_DISCORD_SMOKE_AGENT ||
    "codex";
  const mentionUserId =
    resolveArg("--mention") ||
    process.env.OPENHAND_DISCORD_SMOKE_MENTION_USER_ID ||
    process.env.CLAWDBOT_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction") ||
    process.env.OPENHAND_DISCORD_SMOKE_INSTRUCTION ||
    process.env.CLAWDBOT_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.OPENHAND_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.OPENHAND_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.OPENHAND_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const openhandBin =
    resolveArg("--openhand-bin") || process.env.OPENHAND_DISCORD_SMOKE_OPENHAND_BIN || "openhand";
...
async function openhandCliJson<T>(params: { openhandBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.openhandBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`openhand ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await openhandCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    openhandBin: params.openhandBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "openhand") {
    return await readMessagesWithOpenclaw({
      openhandBin: params.args.openhandBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `openhand-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await openhandCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("openhand message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from OpenHand.`,
        diagnostics: {
          bindingCandidates: [
            {
