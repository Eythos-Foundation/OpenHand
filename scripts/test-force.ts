...
function runTests() {
  const isolatedLock =
    process.env.OPENHAND_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `openhand-gateway.lock.test.${Date.now()}`);
  const result = spawnSync("pnpm", ["vitest", "run"], {
    stdio: "inherit",
    env: {
      ...process.env,
      OPENHAND_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
...
function main() {
  const port = Number.parseInt(process.env.OPENHAND_GATEWAY_PORT ?? `${DEFAULT_PORT}`, 10);
