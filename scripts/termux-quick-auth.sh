...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/openhand/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "OpenHand Re-Auth" -c "After getting key, run: ssh $SERVER '~/openhand/scripts/mobile-reauth.sh'" --id openhand-auth
        ;;
    *)
