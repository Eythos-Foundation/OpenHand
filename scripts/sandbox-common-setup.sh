...
BASE_IMAGE="${BASE_IMAGE:-openhand-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-openhand-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=openhand.sandbox=1)
NOTE
