...
const shouldBuild = (deps) => {
  if (deps.env.OPENHAND_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.OPENHAND_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[openhand] ${message}\n`);
};

const runOpenHand = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["openhand.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
