...
INSTALL_URL="${OPENHAND_INSTALL_URL:-https://openhand.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${OPENHAND_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${OPENHAND_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="openhand"
PACKAGE_NAME="${OPENHAND_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (OPENHAND_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${OPENHAND_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${OPENHAND_INSTALL_LATEST_OUT:-}"
fi
