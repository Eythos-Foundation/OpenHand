...
IMAGE_NAME="openhand-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="openhand-net-e2e-$$"
GW_NAME="openhand-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "OPENHAND_GATEWAY_TOKEN=$TOKEN" \
  -e "OPENHAND_SKIP_CHANNELS=1" \
  -e "OPENHAND_SKIP_GMAIL_WATCHER=1" \
  -e "OPENHAND_SKIP_CRON=1" \
  -e "OPENHAND_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
