 ...
 * `openhand update`. Use it to verify the completion installation flow
...
\`openhand update\`, \`openhand doctor\`, and \`openhand onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# OpenHand Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`openhand completion --install\` later to enable.`));
    return;
  }
