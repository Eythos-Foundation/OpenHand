...
import type { ConfigFileSnapshot } from "./types.openhand.js";
import { OpenHandSchema } from "./zod-schema.js";

const { mapSensitivePaths } = __test__;
const mainSchemaHints = mapSensitivePaths(OpenHandSchema, "", {});
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.openhand/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/openhand/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/openhand/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/openhand/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/openhand/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = OpenHandSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "OpenHandConfig";
    const hints = mainSchemaHints;
