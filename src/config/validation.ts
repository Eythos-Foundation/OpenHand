...
import type { OpenHandConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: OpenHandConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: OpenHandConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: OpenHandConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OpenHandSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as OpenHandConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as OpenHandConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as OpenHandConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as OpenHandConfig,
  };
}
