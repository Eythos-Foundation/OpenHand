...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "openhand.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    OPENHAND_PLUGIN_MANIFEST_CACHE_MS: process.env.OPENHAND_PLUGIN_MANIFEST_CACHE_MS,
  };

  const validateInSuite = (raw: unknown) => validateConfigObjectWithPlugins(raw);

  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-config-plugin-validation-"));
    ...
    process.env.OPENHAND_STATE_DIR = path.join(suiteHome, ".openhand");
    process.env.OPENHAND_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.OPENHAND_STATE_DIR === undefined) {
      delete process.env.OPENHAND_STATE_DIR;
    } else {
      process.env.OPENHAND_STATE_DIR = envSnapshot.OPENHAND_STATE_DIR;
    }
    if (envSnapshot.OPENHAND_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.OPENHAND_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.OPENHAND_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.OPENHAND_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
