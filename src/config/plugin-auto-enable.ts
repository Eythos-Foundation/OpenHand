...
function isWhatsAppConfigured(cfg: OpenHandConfig): boolean {
...
function isGenericChannelConfigured(cfg: OpenHandConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: OpenHandConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: OpenHandConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: OpenHandConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: OpenHandConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: OpenHandConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: OpenHandConfig, pluginId: string): OpenHandConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
