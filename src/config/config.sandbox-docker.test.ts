...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("openhand-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "openhand-sandbox-browser-global" },
      agentBrowser: { network: "openhand-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("openhand-sandbox-browser-agent");
  });
