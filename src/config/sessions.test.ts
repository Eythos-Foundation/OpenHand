...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ OPENHAND_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-openhand",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-openhand#general");
  });
  ...
  it("derives session transcripts dir from OPENHAND_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { OPENHAND_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when OPENHAND_STATE_DIR differs from stored paths", () => {
