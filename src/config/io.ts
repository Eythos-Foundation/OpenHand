...
import type { OpenHandConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareOpenHandVersions } from "./version.js";
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  openhand config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  openhand config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: OpenHandConfig,
  pathSegments: string[],
): { changed: boolean; next: OpenHandConfig } {
...
function coerceConfig(value: unknown): OpenHandConfig {
...
function stampConfigVersion(cfg: OpenHandConfig): OpenHandConfig {
...
function warnIfConfigFromFuture(cfg: OpenHandConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareOpenHandVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer OpenHand (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OpenHandConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): OpenHandConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as OpenHandConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: OpenHandConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as OpenHandConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.OPENHAND_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.OPENHAND_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.OPENHAND_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.OPENHAND_WATCH_SESSION === "string" &&
        deps.env.OPENHAND_WATCH_SESSION.trim().length > 0
          ? deps.env.OPENHAND_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.OPENHAND_WATCH_COMMAND === "string" &&
        deps.env.OPENHAND_WATCH_COMMAND.trim().length > 0
          ? deps.env.OPENHAND_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `OPENHAND_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: OpenHandConfig | null = null;
let runtimeConfigSourceSnapshot: OpenHandConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.OPENHAND_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.OPENHAND_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): OpenHandConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): OpenHandConfig {
