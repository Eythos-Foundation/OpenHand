...
import { buildWebSearchProviderConfig, withTempHome, writeOpenHandConfig } from "./test-helpers.js";
import { OpenHandSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = OpenHandSchema.safeParse({
      $schema: "https://openhand.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://openhand.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = OpenHandSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = OpenHandSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("flags legacy config entries without auto-migrating", async () => {
    await withTempHome(async (home) => {
      await writeOpenHandConfig(home, {
        agents: { list: [{ id: "pi" }] },
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeOpenHandConfig(home, {
        gateway: { bind: "${OPENHAND_BIND}" },
      });

      const prev = process.env.OPENHAND_BIND;
      process.env.OPENHAND_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.OPENHAND_BIND;
        } else {
          process.env.OPENHAND_BIND = prev;
        }
      }
    });
  });
