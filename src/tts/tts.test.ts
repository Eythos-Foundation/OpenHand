...
describe("tts", () => {
  ...
  describe("maybeApplyTtsToPayload", () => {
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.OPENHAND_TTS_PREFS;
      process.env.OPENHAND_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      ...
      try {
        ...
        process.env.OPENHAND_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: OpenHandConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
