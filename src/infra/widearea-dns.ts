...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.OPENHAND_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openhand-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openhand";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openhand");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openhand-gw");
  ...
  records.push(`_openhand-gw._tcp IN PTR ${instanceLabel}._openhand-gw._tcp`);
  records.push(`${instanceLabel}._openhand-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openhand-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openhand-content-hash: ${contentHash}\n${contentBody}`;
}
