...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["OPENHAND_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.OPENHAND_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveOpenHandPackageRoot } = await import("./openhand-root.js"));
    ...
    vi.mocked(resolveOpenHandPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveOpenHandPackageRoot).mockResolvedValue("/opt/openhand");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/openhand/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
