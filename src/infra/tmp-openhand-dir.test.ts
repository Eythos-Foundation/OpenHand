...
import { POSIX_OPENHAND_TMP_DIR, resolvePreferredOpenHandTmpDir } from "./tmp-openhand-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOpenHandTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `openhand-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredOpenHandTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_OPENHAND_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredOpenHandTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredOpenHandTmpDir", () => {
  it("prefers /tmp/openhand when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_OPENHAND_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/openhand when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_OPENHAND_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/openhand when /tmp/openhand is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/openhand when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/openhand is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/openhand is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/openhand is group/other writable", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback OpenHand temp dir/);
  });
