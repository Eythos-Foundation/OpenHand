...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OpenHandConfig | null = null;
  ...
  get(key: string, cfg: OpenHandConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: OpenHandConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: OpenHandConfig): void {
