...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred OpenHand tmp root", async () => {
    const tmpRoot = resolvePreferredOpenHandTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideOpenHand = path.join(os.tmpdir(), "outside-openhand", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
