...
export function triggerOpenHandRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.OPENHAND_SYSTEMD_UNIT,
        process.env.OPENHAND_PROFILE,
      );
  ...
  const label =
    process.env.OPENHAND_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.OPENHAND_PROFILE);
