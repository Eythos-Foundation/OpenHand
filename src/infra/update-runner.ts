...
const DEFAULT_PACKAGE_NAME = "openhand";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["openhand", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "openhand.mjs");
    ...
    const doctorStep = await runStep(
      step("openhand doctor", doctorArgv, gitRoot, { OPENHAND_UPDATE_IN_PROGRESS: "1" }),
    );
