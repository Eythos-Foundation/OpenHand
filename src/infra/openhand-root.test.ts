...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__openhand_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "openhand-root");
...
describe("resolveOpenHandPackageRoot", () => {
  let resolveOpenHandPackageRoot: typeof import("./openhand-root.js").resolveOpenHandPackageRoot;
  let resolveOpenHandPackageRootSync: typeof import("./openhand-root.js").resolveOpenHandPackageRootSync;

  beforeAll(async () => {
    ({ resolveOpenHandPackageRoot, resolveOpenHandPackageRootSync } =
      await import("./openhand-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "openhand");
    const pkgRoot = path.join(project, "node_modules", "openhand");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "openhand" }));

    expect(resolveOpenHandPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "openhand");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "openhand.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "openhand" }));

    expect(resolveOpenHandPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveOpenHandPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-openhand package roots", async () => {
    const pkgRoot = fx("not-openhand");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-openhand" }));

    expect(resolveOpenHandPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveOpenHandPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveOpenHandPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
