...
let ensureOpenHandCliOnPath: typeof import("./path-env.js").ensureOpenHandCliOnPath;

describe("ensureOpenHandCliOnPath", () => {
  const envKeys = [
    "PATH",
    "OPENHAND_PATH_BOOTSTRAPPED",
    "OPENHAND_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureOpenHandCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling openhand exists", () => {
    const tmp = abs("/tmp/openhand-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "openhand");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.OPENHAND_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.OPENHAND_PATH_BOOTSTRAPPED = "1";
    ensureOpenHandCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/openhand-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openhand");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/openhand-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "openhand");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "openhand");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/openhand-path/case-linuxbrew");
