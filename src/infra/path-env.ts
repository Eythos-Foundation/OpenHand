...
function candidateBinDirs(opts: EnsureOpenHandPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `openhand` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "openhand");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.OPENHAND_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "openhand"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `openhand` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOpenHandCliOnPath(opts: EnsureOpenHandPathOpts = {}) {
  if (isTruthyEnvValue(process.env.OPENHAND_PATH_BOOTSTRAPPED)) {
    return;
  }
