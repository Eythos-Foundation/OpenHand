...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.OPENHAND_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.OPENHAND_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set OPENHAND_APNS_TEAM_ID and OPENHAND_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.OPENHAND_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.OPENHAND_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.OPENHAND_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set OPENHAND_APNS_PRIVATE_KEY_P8 or OPENHAND_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading OPENHAND_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createOpenHandPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    openhand: createOpenHandPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
