...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __openhandDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__openhandDiagnosticEventsState) {
    globalStore.__openhandDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__openhandDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: OpenHandConfig): boolean {
