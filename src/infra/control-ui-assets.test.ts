...
vi.mock("./openhand-root.js", () => ({
  resolveOpenHandPackageRoot: vi.fn(async () => null),
  resolveOpenHandPackageRootSync: vi.fn(() => null),
}));
...
let openhandRoot: typeof import("./openhand-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    openhandRoot = await import("./openhand-root.js");
  });
  ...
  it("uses resolveOpenHandPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/openhand");
    (
      openhandRoot.resolveOpenHandPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/openhand"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "openhand" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "openhand.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-openhand");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/openhand-bundle");
    (
      openhandRoot.resolveOpenHandPackageRootSync as unknown as ReturnType<typeof vi.fn>
