...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("openhand-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("openhand-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("openhand-lid-a-", (first) => {
      withTempDirSync("openhand-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.openhand when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "openhand-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers OPENHAND_HOME over HOME", () => {
    vi.stubEnv("OPENHAND_HOME", "/srv/openhand-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/openhand-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENHAND_HOME prefix when OPENHAND_HOME is set", () => {
    vi.stubEnv("OPENHAND_HOME", "/srv/openhand-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/openhand-home")}/.openhand/openhand.json`)).toBe(
      "$OPENHAND_HOME/.openhand/openhand.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPENHAND_HOME replacement when OPENHAND_HOME is set", () => {
    vi.stubEnv("OPENHAND_HOME", "/srv/openhand-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/openhand-home")}/.openhand/openhand.json`),
    ).toBe("config: $OPENHAND_HOME/.openhand/openhand.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/openhand")).toBe(path.resolve(os.homedir(), "openhand"));
  });
  ...
  it("prefers OPENHAND_HOME for tilde expansion", () => {
    vi.stubEnv("OPENHAND_HOME", "/srv/openhand-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/openhand")).toBe(path.resolve("/srv/openhand-home", "openhand"));
