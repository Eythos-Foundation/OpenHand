...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".openhand");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPENHAND_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPENHAND_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPENHAND_STATE_DIR.
