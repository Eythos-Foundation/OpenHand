...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@openhand/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@openhand/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@openhand/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@openhand/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @openhand/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@openhand/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@openhand/plugin-alpha@1.2.3",
        installPath: "/tmp/openhand/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@openhand/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @openhand/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@openhand/plugin-alpha@latest",
      true,
      "/tmp/openhand/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@openhand/plugin-alpha@latest",
      installPath: "/tmp/openhand/extensions/alpha",
      version: "1.2.3",
