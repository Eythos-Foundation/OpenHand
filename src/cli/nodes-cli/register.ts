    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openhand nodes status", "List known nodes with live status."],
          ["openhand nodes pairing pending", "Show pending node pairing requests."],
          ['openhand nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["openhand nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.openhand.ai/cli/nodes")}\n`,
    );
