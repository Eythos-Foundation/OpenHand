...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhand-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhand-bundled-skills-test-"));
    process.env.OPENHAND_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
