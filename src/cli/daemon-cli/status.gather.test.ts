...
const serviceReadCommand = vi.fn(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    OPENHAND_STATE_DIR: "/tmp/openhand-daemon",
    OPENHAND_CONFIG_PATH: "/tmp/openhand-daemon/openhand.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.OPENHAND_STATE_DIR ?? "/tmp/openhand-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.OPENHAND_CONFIG_PATH ?? `${stateDir}/openhand.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/openhand-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENHAND_STATE_DIR = "/tmp/openhand-cli";
    process.env.OPENHAND_CONFIG_PATH = "/tmp/openhand-cli/openhand.json";
