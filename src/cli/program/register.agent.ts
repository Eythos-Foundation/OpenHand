...
${formatHelpExamples([
  ['openhand agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openhand agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openhand agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openhand agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openhand agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'openhand agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openhand.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openhand.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "openhand agents set-identity --identity-file ~/.openhand/workspace/IDENTITY.md --agent main",
