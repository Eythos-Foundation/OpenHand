...
import type { OpenHandConfig, ConfigFileSnapshot } from "../config/types.openhand.js";
...
vi.mock("../infra/openhand-root.js", () => ({
  resolveOpenHandPackageRoot: vi.fn(),
}));
...
const { resolveOpenHandPackageRoot } = await import("../infra/openhand-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/openhand-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as OpenHandConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/openhand-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveOpenHandPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("openhand-update");
    setTty(false);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveOpenHandPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("openhand");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/openhand-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "openhand-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("OpenHand update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as OpenHandConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveOpenHandPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("openhand-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ OPENHAND_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.OPENHAND_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("openhand-update-wizard");
    await withEnvAsync({ OPENHAND_GIT_DIR: tempDir }, async () => {
      setTty(true);
