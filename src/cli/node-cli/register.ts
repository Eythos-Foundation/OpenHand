    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "openhand node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["openhand node status", "Check node host service status."],
          ["openhand node install", "Install the node host service."],
          ["openhand node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.openhand.ai/cli/node")}\n`,
    );
