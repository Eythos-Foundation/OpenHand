...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update OpenHand and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["openhand update", "Update a source checkout (git)"],
        ["openhand update --channel beta", "Switch to beta channel (git + npm)"],
        ["openhand update --channel dev", "Switch to dev channel (git + npm)"],
        ["openhand update --tag beta", "One-off update to a dist-tag or version"],
        ["openhand update --dry-run", "Preview actions without changing anything"],
        ["openhand update --no-restart", "Update without restarting the service"],
        ["openhand update --json", "Output result as JSON"],
        ["openhand update --yes", "Non-interactive (accept downgrade prompts)"],
        ["openhand update wizard", "Interactive update wizard"],
        ["openhand --update", "Shorthand for openhand update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run openhand update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openhand.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openhand update status", "Show channel + version status."],
          ["openhand update status --json", "JSON output."],
          ["openhand update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.openhand.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
