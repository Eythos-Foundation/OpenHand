    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openhand memory status", "Show index and provider status."],
          ["openhand memory index --force", "Force a full reindex."],
          ['openhand memory search --query "deployment notes"', "Search indexed memory entries."],
          ["openhand memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.openhand.ai/cli/memory")}\n`,
    );
