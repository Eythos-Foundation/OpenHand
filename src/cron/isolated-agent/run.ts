...
export type RunCronAgentTurnResult = {
   ...
   * messages.  See: https://github.com/openhand/openhand/issues/15692
   */
  delivered?: boolean;
  /**
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.OPENHAND_TEST_FAST === "1";
  ...
  // correct per-agent paths (e.g. ~/.openhand/agents/<agentId>/agent/).
  ...
  const cfgWithAgentDefaults: OpenHandConfig = {
    ...params.cfg,
    ...
    const fallbackResult = await runWithModelFallback({
      ...
      run: (providerOverride, modelOverride) => {
        ...
        if (isCliProvider(providerOverride, cfgWithAgentDefaults)) {
          ...
          // the configured timeoutSeconds. See: https://github.com/openhand/openhand/issues/29774
