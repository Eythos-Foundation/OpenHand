...
const { makeStorePath } = createCronStoreHarness({ prefix: "openhand-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses OPENHAND_HOME for tilde expansion", () => {
    vi.stubEnv("OPENHAND_HOME", "/srv/openhand-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/openhand-home", "cron", "jobs.json"));
  });
});
