...
function resolveAgentName(cfg: OpenHandConfig, agentId: string) {
...
function resolveAgentModel(cfg: OpenHandConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: OpenHandConfig): AgentSummary[] {
