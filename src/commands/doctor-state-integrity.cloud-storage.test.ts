...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "OpenHand", ".openhand");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/openhand-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".openhand");
  ...
  it("anchors cloud detection to OS homedir when OPENHAND_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".openhand");
    const originalOpenHandHome = process.env.OPENHAND_HOME;
    process.env.OPENHAND_HOME = "/tmp/openhand-home-override";
    ...
    try {
      ...
      if (originalOpenHandHome === undefined) {
        delete process.env.OPENHAND_HOME;
      } else {
        process.env.OPENHAND_HOME = originalOpenHandHome;
      }
    }
  });
