...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under OPENHAND_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("openhand-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-oauth-sync-"));
    ...
    process.env.OPENHAND_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-oauth-nosync-"));
    process.env.OPENHAND_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside OPENHAND_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-oauth-external-"));
    process.env.OPENHAND_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* OPENHAND_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to OPENHAND_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("openhand-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
