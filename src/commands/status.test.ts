...
beforeAll(() => {
  envSnapshot = captureEnv(["OPENHAND_PROFILE"]);
  process.env.OPENHAND_PROFILE = "isolated";
});
...
vi.mock("../infra/openhand-root.js", () => ({
  resolveOpenHandPackageRoot: vi.fn().mockResolvedValue("/tmp/openhand"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openhand/pnpm-lock.yaml",
      markerPath: "/tmp/openhand/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.openhand.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.openhand.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("openhand status --all") ||
          line.includes("openhand --profile isolated status --all"),
      ),
    ).toBe(true);
  });

  it("shows gateway auth when reachable", async () => {
    await withEnvVar("OPENHAND_GATEWAY_TOKEN", "abcd1234", async () => {
