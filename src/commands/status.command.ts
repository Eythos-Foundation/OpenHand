...
export async function statusCommand(
  ...
  runtime.log(theme.heading("OpenHand status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`openhand devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("openhand devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("openhand devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openhand security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openhand security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openhand.ai/faq");
  runtime.log("Troubleshooting: https://docs.openhand.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openhand status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openhand logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openhand status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openhand gateway probe")}`);
  }
}
