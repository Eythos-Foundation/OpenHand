...
export async function loadValidConfigOrThrow(): Promise<OpenHandConfig> {
...
export async function updateConfig(
  mutator: (cfg: OpenHandConfig) => OpenHandConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: OpenHandConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: OpenHandConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("openhand agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
