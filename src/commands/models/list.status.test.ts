...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveOpenHandAgentDir: vi.fn().mockReturnValue("/tmp/openhand-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/openhand-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/openhand-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveOpenHandAgentDir: mocks.resolveOpenHandAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/openhand-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveOpenHandAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/openhand-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/openhand-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/openhand-agent-custom");
