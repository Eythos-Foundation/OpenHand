...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveOpenHandAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openhand models auth setup-token")}\` or \`${formatCliCommand("openhand configure")}\`.`
          : `Run \`${formatCliCommand("openhand configure")}\` or set an API key env var.`;
