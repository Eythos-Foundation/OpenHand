...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "openhand-agent-acp-" });
}

function createAcpEnabledConfig(home: string, storePath: string): OpenHandConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
    ...
    agents: {
      defaults: {
        ...
        workspace: path.join(home, "openhand"),
      },
    },
...
function mockConfigWithAcpOverrides(
  home: string,
  storePath: string,
  acpOverrides: Partial<NonNullable<OpenHandConfig["acp"]>>,
) {
  ...
  it.each([
    {
      ...
      acpOverrides: { enabled: false } satisfies Partial<NonNullable<OpenHandConfig["acp"]>>,
    },
    {
      name: "blocks ACP turns when ACP dispatch is disabled by policy",
      acpOverrides: {
        dispatch: { enabled: false },
      } satisfies Partial<NonNullable<OpenHandConfig["acp"]>>,
    },
