...
export function normalizeCompatibilityConfigValues(cfg: OpenHandConfig): {
  config: OpenHandConfig;
  changes: string[];
} {
  const changes: string[] = [];
  let next: OpenHandConfig = cfg;
  ...
  const seedMissingDefaultAccountsFromSingleAccountBase = () => {
    ...
    next = {
      ...next,
      channels: nextChannels as OpenHandConfig["channels"],
    };
  };
  ...
  const normalizeBrowserSsrFPolicyAlias = () => {
    ...
    next = {
      ...next,
      browser: migratedBrowser as OpenHandConfig["browser"],
    };
    changes.push(
