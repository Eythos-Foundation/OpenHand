...
const SEARCH_TOOL = "https://docs.openhand.ai/mcp.SearchOpenHand";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openhand.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('openhand docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openhand.ai/");
      runtime.log(`Search: ${formatCliCommand('openhand docs "your query"')}`);
    }
    return;
  }
