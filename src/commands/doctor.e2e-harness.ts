...
export const resolveOpenHandPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadOpenHandPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveOpenHandPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.OPENHAND_STATE_DIR;
  originalUpdateInProgress = process.env.OPENHAND_UPDATE_IN_PROGRESS;
  process.env.OPENHAND_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openhand-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.OPENHAND_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.OPENHAND_UPDATE_IN_PROGRESS;
  } else {
    process.env.OPENHAND_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
