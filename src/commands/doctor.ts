...
function resolveMode(cfg: OpenHandConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("OpenHand doctor");

  const root = await resolveOpenHandPackageRoot({
  ...
  let cfg: OpenHandConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("openhand configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("openhand config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("openhand setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("openhand doctor --fix")}" to apply changes.`);
  }
