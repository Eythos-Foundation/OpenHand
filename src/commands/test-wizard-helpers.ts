...
export function requireOpenHandAgentDir(): string {
  const agentDir = process.env.OPENHAND_AGENT_DIR;
  if (!agentDir) {
    throw new Error("OPENHAND_AGENT_DIR not set");
  }
  return agentDir;
}
