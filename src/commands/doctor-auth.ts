...
function pruneAuthProfiles(
  cfg: OpenHandConfig,
  profileIds: Set<string>,
): { next: OpenHandConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("openhand models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("openhand models auth setup-token")} or ${formatCliCommand(
      "openhand configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "openhand models auth login --provider openai-codex",
    )} or ${formatCliCommand("openhand configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("openhand configure")}\` or \`${formatCliCommand("openhand onboard")}\`.`;
}
