...
import type { OpenHandConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: OpenHandConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openhand security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.openhand/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("openhand approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.openhand.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openhand configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openhand config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openhand doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("openhand config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('openhand config set session.dmScope "per-channel-peer"') +
