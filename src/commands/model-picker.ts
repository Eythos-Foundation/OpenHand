...
type PromptDefaultModelResult = { model?: string; config?: OpenHandConfig };
...
function resolveConfiguredModelRaw(cfg: OpenHandConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: OpenHandConfig): string[] {
...
export function applyPrimaryModel(cfg: OpenHandConfig, model: string): OpenHandConfig {
...
export function applyModelAllowlist(cfg: OpenHandConfig, models: string[]): OpenHandConfig {
  const defaults = cfg.agents?.defaults;
