...
function listAgents(cfg: OpenHandConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: OpenHandConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<OpenHandConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: OpenHandConfig): AgentLookupCache {
...
function pickFirstExistingAgentId(cfg: OpenHandConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OpenHandConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
  byChannelAccountIndex: Map<string, EvaluatedBindingsIndex>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<OpenHandConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  OpenHandConfig,
  {
    bindingsRef: OpenHandConfig["bindings"];
    agentsRef: OpenHandConfig["agents"];
    sessionRef: OpenHandConfig["session"];
...
function resolveRouteCacheForConfig(cfg: OpenHandConfig): Map<string, ResolvedAgentRoute> {
