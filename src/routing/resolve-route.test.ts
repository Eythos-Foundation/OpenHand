...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: OpenHandConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: OpenHandConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/openhand-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<OpenHandConfig["bindings"]>[number];
