...
homepage: https://docs.openhand.ai/automation/hooks#command-logger
metadata:
  {
    "openhand":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.openhand/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.openhand/logs/commands.log ~/.openhand/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/openhand`:
   ```
   /home/username/.openhand/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
