...
This directory contains hooks that ship with OpenHand. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.openhand/openhand.json`:
...
2. Restart gateway: `pkill -9 -f 'openhand.*gateway' && pnpm openhand gateway`
3. Enable the hook: `openhand hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.openhand.ai/automation/hooks
