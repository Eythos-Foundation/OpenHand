...
import { __testing, loadOpenHandPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openhand-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.OPENHAND_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "openhand.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.OPENHAND_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadOpenHandPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          openhand: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
  ...
  process.env.OPENHAND_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.OPENHAND_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadOpenHandPlugins>) {
...
function useNoBundledPlugins() {
  process.env.OPENHAND_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadOpenHandPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.OPENHAND_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.OPENHAND_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadOpenHandPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadOpenHandPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadOpenHandPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadOpenHandPlugins(options);
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ OPENHAND_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
