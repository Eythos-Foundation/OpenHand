...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OpenHandPluginDefinition;
  register?: OpenHandPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as OpenHandPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as OpenHandPluginDefinition;
...
export function loadOpenHandPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverOpenHandPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "openhand/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "openhand/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: OpenHandPluginModule | null = null;
    try {
      mod = getJiti()(safeSource) as OpenHandPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
