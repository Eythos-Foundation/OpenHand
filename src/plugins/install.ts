...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing openhand.extensions; update the plugin package to include openhand.extensions (for example ["./dist/index.js"]). See https://docs.openhand.ai/help/troubleshooting#plugin-install-fails-with-missing-openhand-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_OPENHAND_EXTENSIONS: "missing_openhand_extensions",
  EMPTY_OPENHAND_EXTENSIONS: "empty_openhand_extensions",
...
function ensureOpenHandExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_OPENHAND_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json openhand.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_OPENHAND_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureOpenHandExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from openhand.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-openhand"), the plugin registry
  ...
  try {
    ...
    if (scanSummary.critical > 0) {
      ...
      logger.warn?.(
        `Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "openhand security audit --deep" for details.`,
      );
    }
  } catch (err) {
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "openhand security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "openhand-npm-pack-",
    spec,
    timeoutMs,
