...
const createTempDir = () => tempDirs.make("openhand-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets OPENHAND_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "openhand",
    });

    expect(env.OPENHAND_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("openhand");
  });

  it("overrides pre-existing OPENHAND_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      OPENHAND_SHELL: "wrong",
    });
    expect(env.OPENHAND_SHELL).toBe("acp-client");
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "openhand", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "openhand", "dist", "entry.js");
    const shimPath = path.join(dir, "openhand.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\openhand\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/openhand-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/openhand-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/openhand-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
