...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["OPENHAND_GATEWAY_TOKEN", "OPENHAND_GATEWAY_PASSWORD"]);
  ...
  it("uses OPENHAND_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ OPENHAND_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers OPENHAND_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ OPENHAND_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
