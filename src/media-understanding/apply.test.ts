...
const TEMP_MEDIA_PREFIX = "openhand-media-";
...
function createGroqAudioConfig(): OpenHandConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): OpenHandConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): OpenHandConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: OpenHandConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredOpenHandTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        OPENHAND_AGENT_DIR: isolatedAgentDir,
