...
describe("shared/frontmatter", () => {
  ...
  test("resolveOpenHandManifestBlock parses JSON5 metadata and picks openhand block", () => {
    const frontmatter = {
      metadata: "{ openhand: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveOpenHandManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveOpenHandManifestBlock returns undefined for invalid input", () => {
    expect(resolveOpenHandManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveOpenHandManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveOpenHandManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
