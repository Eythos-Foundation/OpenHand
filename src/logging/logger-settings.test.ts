...
let originalOpenHandLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.OPENHAND_TEST_FILE_LOG;
  originalOpenHandLogLevel = process.env.OPENHAND_LOG_LEVEL;
  delete process.env.OPENHAND_TEST_FILE_LOG;
  delete process.env.OPENHAND_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.OPENHAND_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalOpenHandLogLevel === undefined) {
    delete process.env.OPENHAND_LOG_LEVEL;
  } else {
    process.env.OPENHAND_LOG_LEVEL = originalOpenHandLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
