...
function ensureNodePathEnv(): string {
  ensureOpenHandCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
export async function resolveNodeHostGatewayCredentials(params: {
  ...
  const token =
    normalizeSecretInputString(env.OPENHAND_GATEWAY_TOKEN) ??
  ...
  const shouldResolveConfiguredPassword =
    !normalizeSecretInputString(env.OPENHAND_GATEWAY_PASSWORD) &&
    !tokenCanWin &&
