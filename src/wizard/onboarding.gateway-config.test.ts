...
describe("configureGatewayForOnboarding", () => {
  ...
  it("prefers OPENHAND_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.OPENHAND_GATEWAY_TOKEN;
    process.env.OPENHAND_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    const previous = process.env.OPENHAND_GATEWAY_PASSWORD;
    process.env.OPENHAND_GATEWAY_PASSWORD = "gateway-secret";
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "OPENHAND_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.OPENHAND_GATEWAY_PASSWORD = previous;
      }
    }
  });
});
