...
function createReplyConfig(home: string, streamMode?: "block"): OpenHandConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("openhand-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("OPENHAND_TEST_FAST", "1");
