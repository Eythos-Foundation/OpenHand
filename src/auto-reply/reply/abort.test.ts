...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop openhand!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@openhand_bot", { botUsername: "openhand_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@openhand_bot", { botUsername: "openhand_bot" })).toBe(true);
