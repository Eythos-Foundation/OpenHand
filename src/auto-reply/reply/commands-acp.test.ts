...
function createDiscordParams(commandBody: string, cfg: OpenHandConfig = baseCfg) {
...
function createThreadParams(commandBody: string, cfg: OpenHandConfig = baseCfg) {
  const params = createDiscordParams(commandBody, cfg);
  params.ctx.MessageThreadId = defaultThreadId;
  return params;
}

async function runDiscordAcpCommand(commandBody: string, cfg: OpenHandConfig = baseCfg) {
  return handleAcpCommand(createDiscordParams(commandBody, cfg), true);
}

async function runThreadAcpCommand(commandBody: string, cfg: OpenHandConfig = baseCfg) {
