...
export function isCommandEnabled(cfg: OpenHandConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: OpenHandConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as OpenHandConfig),
...
export function getCommandDetection(_cfg?: OpenHandConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: OpenHandConfig) {
