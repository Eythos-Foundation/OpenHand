...
// Prefix for OpenHand-generated action IDs to scope our handler
const OPENHAND_ACTION_PREFIX = "openhand:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${OPENHAND_ACTION_PREFIX}`);

  // Handle OpenHand modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
