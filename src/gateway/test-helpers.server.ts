...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "OPENHAND_SKIP_BROWSER_CONTROL_SERVER",
  "OPENHAND_SKIP_GMAIL_WATCHER",
  "OPENHAND_SKIP_CANVAS_HOST",
  "OPENHAND_BUNDLED_PLUGINS_DIR",
  "OPENHAND_SKIP_CHANNELS",
  "OPENHAND_SKIP_PROVIDERS",
  "OPENHAND_SKIP_CRON",
  "OPENHAND_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.OPENHAND_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.OPENHAND_SKIP_GMAIL_WATCHER = "1";
  process.env.OPENHAND_SKIP_CANVAS_HOST = "1";
  process.env.OPENHAND_SKIP_CHANNELS = "1";
  process.env.OPENHAND_SKIP_PROVIDERS = "1";
  process.env.OPENHAND_SKIP_CRON = "1";
  process.env.OPENHAND_TEST_MINIMAL_GATEWAY = "1";
  process.env.OPENHAND_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "openhand-test-no-bundled-extensions")
    : "openhand-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".openhand-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".openhand-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
const CONNECT_CHALLENGE_NONCE_KEY = "__openhandTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__openhandTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["OPENHAND_GATEWAY_TOKEN"]);
  const prev = process.env.OPENHAND_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.OPENHAND_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.OPENHAND_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
