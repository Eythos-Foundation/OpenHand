...
import { createOpenHandTools } from "../agents/openhand-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-openhand-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-openhand-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-openhand-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-openhand-thread-id")?.trim() || undefined;
  ...
  const allTools = createOpenHandTools({
