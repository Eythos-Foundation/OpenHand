...
let lastCreateOpenHandToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/openhand-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateOpenHandToolsContext?.agentTo,
          agentThreadId: lastCreateOpenHandToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createOpenHandTools: (ctx: Record<string, unknown>) => {
      lastCreateOpenHandToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateOpenHandToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-openhand-message-to": "channel:24514",
        "x-openhand-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
