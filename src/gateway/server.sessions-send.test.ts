...
const { createOpenHandTools } = await import("../agents/openhand-tools.js");
...
type SessionSendTool = ReturnType<typeof createOpenHandTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createOpenHandTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["OPENHAND_GATEWAY_PORT", "OPENHAND_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.OPENHAND_GATEWAY_PORT = String(gatewayPort);
  process.env.OPENHAND_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("OPENHAND_CONFIG_PATH missing in gateway test environment");
      }
