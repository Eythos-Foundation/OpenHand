...
export function resolveHooksConfig(cfg: OpenHandConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OpenHandConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-openhand-token"] === "string"
      ? req.headers["x-openhand-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
