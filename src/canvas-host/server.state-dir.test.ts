...
describe("canvas host state dir defaults", () => {
  it("uses OPENHAND_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("openhand-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("OpenHand Canvas");
      } finally {
        await handler.close();
      }
    });
