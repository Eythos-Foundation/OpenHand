...
const RELAY_AUTH_HEADER = "x-openhand-relay-token";
  ...
  const initPromise = (async (): Promise<ChromeExtensionRelayServer> => {
    ...
    const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
      switch (cmd.method) {
        case "Browser.getVersion":
          return {
            protocolVersion: "1.3",
            product: "Chrome/OpenHand-Extension-Relay",
            revision: "0",
            userAgent: "OpenHand-Extension-Relay",
            jsVersion: "V8",
          };
    ...
    const server = createServer((req, res) => {
      ...
      if (
        ...
        const payload: Record<string, unknown> = {
          Browser: "OpenHand/extension-relay",
    ...
    try {
      ...
      if (
        ...
        (await probeAuthenticatedOpenHandRelay({
