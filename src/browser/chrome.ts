...
export function resolveOpenHandUserDataDir(profileName = DEFAULT_OPENHAND_BROWSER_PROFILE_NAME) {
...
export async function launchOpenHandChrome(
  ...
  const userDataDir = resolveOpenHandUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_OPENHAND_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateOpenHandProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`openhand browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopOpenHandChrome(
