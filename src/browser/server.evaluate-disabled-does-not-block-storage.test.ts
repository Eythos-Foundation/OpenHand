  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          openhand: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.OPENHAND_GATEWAY_PORT;
    process.env.OPENHAND_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.OPENHAND_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.OPENHAND_GATEWAY_PASSWORD;
