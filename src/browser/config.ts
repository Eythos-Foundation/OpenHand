import type { BrowserConfig, BrowserProfileConfig, OpenHandConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_OPENHAND_BROWSER_ENABLED,
...
function normalizeHexColor(raw: string | undefined) {
  ...
  if (!value) {
    return DEFAULT_OPENHAND_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "openhand" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_OPENHAND_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_OPENHAND_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an OpenHand browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for openhand/openhand CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: OpenHandConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_OPENHAND_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_OPENHAND_BROWSER_PROFILE_NAME]
        ? DEFAULT_OPENHAND_BROWSER_PROFILE_NAME
        : "chrome");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "openhand";
