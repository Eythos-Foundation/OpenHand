...
function localOpenHandProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "openhand",
...
function createLocalOpenHandResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localOpenHandProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveOpenHandUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openhand-reset-"));
    const profileDir = path.join(tempRoot, "openhand");
    ...
    const ops = createLocalOpenHandResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveOpenHandUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openhand-reset-no-own-"));
    const profileDir = path.join(tempRoot, "openhand");
    fs.mkdirSync(profileDir, { recursive: true });
