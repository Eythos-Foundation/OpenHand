...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".openhand-profile-decorated");
}
 ...
 */
export function decorateOpenHandProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_OPENHAND_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_OPENHAND_BROWSER_COLOR).toUpperCase();
