...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.OPENHAND_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    OPENHAND_PROFILE: profile,
    OPENHAND_GATEWAY_PORT: String(port),
    OPENHAND_GATEWAY_TOKEN: token,
    OPENHAND_LAUNCHD_LABEL: resolvedLaunchdLabel,
    OPENHAND_SYSTEMD_UNIT: systemdUnit,
    OPENHAND_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    OPENHAND_SERVICE_KIND: GATEWAY_SERVICE_KIND,
...
export function buildNodeServiceEnvironment(params: {
  ...
  const sharedEnv = resolveSharedServiceEnvironmentFields(env, platform);
  const gatewayToken =
    env.OPENHAND_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim() || undefined;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    OPENHAND_GATEWAY_TOKEN: gatewayToken,
    OPENHAND_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    OPENHAND_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    OPENHAND_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    OPENHAND_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    OPENHAND_LOG_PREFIX: "node",
    OPENHAND_SERVICE_MARKER: NODE_SERVICE_MARKER,
    OPENHAND_SERVICE_KIND: NODE_SERVICE_KIND,
    OPENHAND_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  return {
    ...
    OPENHAND_STATE_DIR: sharedEnv.stateDir,
    OPENHAND_CONFIG_PATH: sharedEnv.configPath,
  };
}
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.OPENHAND_STATE_DIR;
  const configPath = env.OPENHAND_CONFIG_PATH;
