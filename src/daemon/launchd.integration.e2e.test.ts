...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openhand-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      OPENHAND_LAUNCHD_LABEL: `ai.openhand.launchd-int-${testId}`,
      OPENHAND_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
