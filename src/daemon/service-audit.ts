...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.OPENHAND_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service OPENHAND_GATEWAY_TOKEN does not match gateway.auth.token in openhand.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `openhand gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
