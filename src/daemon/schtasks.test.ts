...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when OPENHAND_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".openhand", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when OPENHAND_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", OPENHAND_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".openhand-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers OPENHAND_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        OPENHAND_PROFILE: "rescue",
        OPENHAND_STATE_DIR: "C:\\State\\openhand",
      },
      expected: path.join("C:\\State\\openhand", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", OPENHAND_PROFILE: "default" },
      expected: path.join("/home/test", ".openhand", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-schtasks-test-"));
    try {
      const extraEnv = typeof options.env === "function" ? options.env(tmpDir) : options.env;
      const env = {
        USERPROFILE: tmpDir,
        OPENHAND_PROFILE: "default",
        ...extraEnv,
      };
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem OpenHand Gateway",
          "cd /d C:\\Projects\\openhand",
          "set NODE_ENV=production",
          "set OPENHAND_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\openhand",
          environment: {
            NODE_ENV: "production",
            OPENHAND_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\openhand\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\openhand\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\OpenHand Share\\node.exe" "\\\\fileserver\\OpenHand Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\OpenHand Share\\node.exe",
            "\\\\fileserver\\OpenHand Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from OPENHAND_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ OPENHAND_STATE_DIR: path.join(tmpDir, "custom-state") }),
