...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENHAND_GATEWAY_PORT).toBe("18789");
    expect(env.OPENHAND_GATEWAY_TOKEN).toBe("secret");
    expect(env.OPENHAND_SERVICE_MARKER).toBe("openhand");
    expect(env.OPENHAND_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENHAND_SERVICE_VERSION).toBe("string");
    expect(env.OPENHAND_SYSTEMD_UNIT).toBe("openhand-gateway.service");
    if (process.platform === "darwin") {
      expect(env.OPENHAND_LAUNCHD_LABEL).toBe("ai.openhand.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", OPENHAND_PROFILE: "work" },
      port: 18789,
    });
    expect(env.OPENHAND_SYSTEMD_UNIT).toBe("openhand-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.OPENHAND_LAUNCHD_LABEL).toBe("ai.openhand.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through OPENHAND_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", OPENHAND_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.OPENHAND_GATEWAY_TOKEN).toBe("node-token");
  });

  it("maps legacy CLAWDBOT_GATEWAY_TOKEN to OPENHAND_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", CLAWDBOT_GATEWAY_TOKEN: " legacy-token " },
    });
    expect(env.OPENHAND_GATEWAY_TOKEN).toBe("legacy-token");
  });

  it("prefers OPENHAND_GATEWAY_TOKEN over legacy CLAWDBOT_GATEWAY_TOKEN", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        OPENHAND_GATEWAY_TOKEN: "openhand-token",
        CLAWDBOT_GATEWAY_TOKEN: "legacy-token",
      },
    });
    expect(env.OPENHAND_GATEWAY_TOKEN).toBe("openhand-token");
  });

  it("omits OPENHAND_GATEWAY_TOKEN when both token env vars are empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
    ...
    expect(env.OPENHAND_GATEWAY_TOKEN).toBeUndefined();
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openhand"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OPENHAND_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openhand-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", OPENHAND_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".openhand"));
  });

  it("uses OPENHAND_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OPENHAND_STATE_DIR: "/var/lib/openhand" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/openhand"));
  });

  it("expands ~ in OPENHAND_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OPENHAND_STATE_DIR: "~/openhand-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/openhand-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OPENHAND_STATE_DIR: "C:\\State\\openhand" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\openhand");
  });
});
