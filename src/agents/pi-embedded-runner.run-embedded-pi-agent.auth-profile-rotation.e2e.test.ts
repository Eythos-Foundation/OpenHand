...
vi.mock("./models-config.js", async (importOriginal) => {
  ...
  return {
    ...mod,
    ensureOpenHandModelsJson: vi.fn(async () => ({ wrote: false })),
  };
});
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): OpenHandConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): OpenHandConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): OpenHandConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-workspace-"));
    const now = Date.now();
