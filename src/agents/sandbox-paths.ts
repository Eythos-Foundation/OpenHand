...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const openHandTmpDir = path.resolve(resolvePreferredOpenHandTmpDir());
  if (!isPathInside(openHandTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: openHandTmpDir });
  return resolved;
}
