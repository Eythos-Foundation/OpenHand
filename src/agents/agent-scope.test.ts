...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: OpenHandConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/openhand" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: OpenHandConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: OpenHandConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: OpenHandConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: OpenHandConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: OpenHandConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: OpenHandConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: OpenHandConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/openhand");
  });

  it("uses OPENHAND_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "openhand-home");
    vi.stubEnv("OPENHAND_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as OpenHandConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".openhand", "workspace"));
  });

  it("uses OPENHAND_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "openhand-home");
    vi.stubEnv("OPENHAND_HOME", home);
    // Clear state dir so it falls back to OPENHAND_HOME
    vi.stubEnv("OPENHAND_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as OpenHandConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".openhand", "agents", "main", "agent"));
  });
});
