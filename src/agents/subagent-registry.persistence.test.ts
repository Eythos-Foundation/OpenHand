...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["OPENHAND_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-subagent-"));
  ...
  it("uses isolated temp state when OPENHAND_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "openhand-test-state"));
  });
});
