...
function requireGatewayTool(agentSessionKey?: string) {
  const tool = createOpenHandTools({
...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-test-"));

    try {
      await withEnvAsync(
        { OPENHAND_STATE_DIR: stateDir, OPENHAND_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: openhand --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/openhand" } }\n}\n';
