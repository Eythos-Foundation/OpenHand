    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "openhand.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "openhand.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "openhand.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("openhand-");
  const acpxRoot = await tempDirs.make("openhand-acpx-plugin-");
  const helperRoot = await tempDirs.make("openhand-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("openhand-");
  const pluginRoot = await tempDirs.make("openhand-plugin-");
  const outsideDir = await tempDirs.make("openhand-outside-");
