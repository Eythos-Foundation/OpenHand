...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("openhand-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("openhand-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("openhand-sbx-");
