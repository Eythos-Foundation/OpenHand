...
const GOOGLE_LIVE_SINGLE_KEY = "OPENHAND_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "OPENHAND_LIVE_ANTHROPIC_KEY",
    listVar: "OPENHAND_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "OPENHAND_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `OPENHAND_LIVE_${base}_KEY`;
