...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openhand.sandbox=1");
  args.push("--label", `openhand.sessionKey=${params.scopeKey}`);
  args.push("--label", `openhand.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openhand.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "openhand.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openhand sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openhand sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openhand sandbox recreate --all");
}
