...
import { resolvePreferredOpenHandTmpDir } from "../infra/tmp-openhand-dir.js";
...
async function withOutsideHardlinkInOpenHandTmp<T>(
  params: {
    openHandTmpDir: string;
  ...
  const hardlinkPath = path.join(params.openHandTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.openHandTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.openHandTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.openHandTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const openHandTmpDir = resolvePreferredOpenHandTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred OpenHand tmp root",
      media: path.join(openHandTmpDir, "image.png"),
      expected: path.join(openHandTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred OpenHand tmp root",
      media: pathToFileURL(path.join(openHandTmpDir, "photo.png")).href,
      expected: path.join(openHandTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred OpenHand tmp root",
      media: path.join(openHandTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(openHandTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(openHandTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside openhand tmp root",
      media: path.join(os.tmpdir(), "outside-openhand", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked OpenHand tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(openHandTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(openHandTmpDir, { recursive: true });
      const symlinkPath = path.join(openHandTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked OpenHand tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenHandTmp(
      {
        openHandTmpDir,
  ...
  it("rejects symlinked OpenHand tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOpenHandTmp(
