...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.OPENHAND_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".openhand", `workspace-${profile}`);
  }
  return path.join(home, ".openhand", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".openhand";
