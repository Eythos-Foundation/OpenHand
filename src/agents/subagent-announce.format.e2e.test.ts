...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/openhand/openhand/issues/31298
    previousFastTestEnv = process.env.OPENHAND_TEST_FAST;
  ...
  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.OPENHAND_TEST_FAST;
      return;
    }
    process.env.OPENHAND_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // OPENHAND_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("OpenHand runtime context (internal):");
