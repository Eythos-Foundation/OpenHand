...
type ModelsConfig = NonNullable<OpenHandConfig["models"]>;
...
export async function ensureOpenHandModelsJson(
  config?: OpenHandConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = config ?? loadConfig();
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveOpenHandAgentDir();
