...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    openhand: process.env.OPENHAND_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.openhand === undefined) {
      delete process.env.OPENHAND_GATEWAY_TOKEN;
    } else {
      process.env.OPENHAND_GATEWAY_TOKEN = envSnapshot.openhand;
    }
  ...
  it("uses OPENHAND_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.OPENHAND_GATEWAY_TOKEN = "local-env-token";
