...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): OpenHandConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as OpenHandConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): OpenHandConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as OpenHandConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: OpenHandConfig, accountId = "media") {
    await handleTelegramAction(
