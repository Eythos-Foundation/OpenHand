...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses OPENHAND_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      OPENHAND_HOME: "/srv/openhand-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/openhand-home"), ".openhand", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".openhand", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("openhand-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-workspace-hardlink-"));
    try {
