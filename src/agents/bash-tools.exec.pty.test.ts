...
test("exec sets OPENHAND_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-openhand-shell", {
    command: "node -e \"process.stdout.write(process.env.OPENHAND_SHELL || '')\"",
    pty: true,
  });
