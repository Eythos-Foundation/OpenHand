...
type CoreToolDefinition = {
  ...
  includeInOpenHandGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const openhandTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInOpenHandGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:openhand": openhandTools,
