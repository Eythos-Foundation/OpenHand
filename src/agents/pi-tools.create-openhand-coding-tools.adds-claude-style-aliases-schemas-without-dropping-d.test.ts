...
describe("createOpenHandCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openhand-read-"));
    try {
      ...
      const contents = "Hello from openhand read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createOpenHandCodingTools({ sandbox });
