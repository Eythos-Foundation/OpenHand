...
function resolveAuth(cfg: OpenHandConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.OPENHAND_GATEWAY_PASSWORD?.trim() ||
...
async function resolveGatewayPasswordSecretRef(
  ...
  const hasPasswordEnvCandidate = Boolean(
    env.OPENHAND_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim(),
  );
  ...
  if (mode !== "password") {
    const hasTokenCandidate =
      Boolean(env.OPENHAND_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim()) ||
