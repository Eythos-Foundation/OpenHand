...
function resolveVoiceTtsConfig(params: { cfg: OpenHandConfig; override?: TtsConfig }): {
  cfg: OpenHandConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredOpenHandTmpDir(), "discord-voice-"));
