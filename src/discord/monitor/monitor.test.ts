...
describe("agent components", () => {
  const createCfg = (): OpenHandConfig => ({}) as OpenHandConfig;
...
describe("discord component interactions", () => {
  const createCfg = (): OpenHandConfig =>
    ({
      channels: {
        discord: {
          replyToMode: "first",
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/openhand-sessions-test.json");
  });
