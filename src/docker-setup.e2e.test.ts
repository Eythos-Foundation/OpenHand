...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "openhand-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    OPENHAND_GATEWAY_TOKEN: "test-token",
    OPENHAND_CONFIG_DIR: join(sandbox.rootDir, "config"),
    OPENHAND_WORKSPACE_DIR: join(sandbox.rootDir, "openhand"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENHAND_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      OPENHAND_EXTRA_MOUNTS: undefined,
      OPENHAND_HOME_VOLUME: "openhand-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENHAND_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("OPENHAND_EXTRA_MOUNTS=");
    expect(envFile).toContain("OPENHAND_HOME_VOLUME=openhand-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("openhand-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("openhand-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg OPENHAND_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(log).toContain("run --rm openhand-cli onboard --mode local --no-install-daemon");
    expect(log).toContain("run --rm openhand-cli config set gateway.mode local");
    expect(log).toContain("run --rm openhand-cli config set gateway.bind lan");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENHAND_CONFIG_DIR: configDir,
      OPENHAND_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when OPENHAND_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    await writeFile(
      join(configDir, "openhand.json"),
      JSON.stringify({ gateway: { auth: { mode: "token", token: "config-token-123" } } }),
    );

    const result = runDockerSetup(activeSandbox, {
      OPENHAND_GATEWAY_TOKEN: undefined,
      OPENHAND_CONFIG_DIR: configDir,
      OPENHAND_WORKSPACE_DIR: workspaceDir,
    });
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENHAND_GATEWAY_TOKEN=config-token-123");
  });

  it("treats OPENHAND_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("OPENHAND_SANDBOX=");

    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg OPENHAND_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg OPENHAND_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      OPENHAND_SANDBOX: "1",
      DOCKER_STUB_FAIL_MATCH: "--entrypoint docker openhand-gateway --version",
    });
  ...
  it("skips sandbox gateway restart when sandbox config writes fail", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        OPENHAND_SANDBOX: "1",
        OPENHAND_DOCKER_SOCKET: socketPath,
      ...
      const gatewayStarts = log
        ...
        .filter(
          (line) =>
            ...
            line.includes(" up -d") &&
            line.includes("openhand-gateway"),
        );
      expect(gatewayStarts).toHaveLength(2);
      expect(log).toContain(
        "run --rm --no-deps openhand-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("up -d --force-recreate openhand-gateway"));
  ...
  it("rejects injected multiline OPENHAND_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENHAND_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("OPENHAND_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid OPENHAND_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENHAND_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid OPENHAND_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      OPENHAND_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("OPENHAND_HOME_VOLUME must match");
  });
  ...
  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:openhand-gateway"');
