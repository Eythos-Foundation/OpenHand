...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: OpenHandConfig, policy: ChannelAccessPolicy) => OpenHandConfig;
  resolveAllowlist: (params: { cfg: OpenHandConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: OpenHandConfig; resolved: TResolved }) => OpenHandConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: OpenHandConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: OpenHandConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: OpenHandConfig, policy: ChannelAccessPolicy): OpenHandConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: OpenHandConfig, policy: ChannelAccessPolicy): OpenHandConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: OpenHandConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: OpenHandConfig; resolved: string[] }) => {
      calls.push("apply");
