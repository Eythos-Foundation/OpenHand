...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@openhand/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "openhand-catalog-"));
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: OpenHandConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
