...
import { discoverOpenHandPlugins } from "../../plugins/discovery.js";
import type { OpenHandPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, OpenHandPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["OPENHAND_PLUGIN_CATALOG_PATHS", "OPENHAND_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<OpenHandPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: OpenHandPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: OpenHandPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverOpenHandPlugins({ workspaceDir: options.workspaceDir });
