...
function telegramCfg(): OpenHandConfig {
  return { channels: { telegram: { botToken: "tok" } } } as OpenHandConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: OpenHandConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as OpenHandConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as OpenHandConfig;
...
function createSignalAccountOverrideCfg(): OpenHandConfig {
  return {
    channels: {
      signal: {
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as OpenHandConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as OpenHandConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as OpenHandConfig,
        accountId: undefined,
        params: {
