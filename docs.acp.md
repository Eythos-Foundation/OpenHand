...
This document describes how the OpenHand ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`openhand acp` exposes an ACP agent over stdio and forwards prompts to a running
OpenHand Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `openhand acp` and speaks ACP messages over stdio.
...
`openhand acp` resolves the Gateway URL and auth from CLI flags or config:
