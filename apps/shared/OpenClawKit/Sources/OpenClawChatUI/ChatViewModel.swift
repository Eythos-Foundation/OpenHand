...
private let chatUILogger = Logger(subsystem: "ai.openhand", category: "OpenHandChatUI")

@MainActor
@Observable
public final class OpenHandChatViewModel {
    public private(set) var messages: [OpenHandChatMessage] = []
    ...
    public var attachments: [OpenHandPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OpenHandChatPendingToolCall] = []
    public private(set) var sessions: [OpenHandChatSessionEntry] = []
    private let transport: any OpenHandChatTransport
    ...
    private var pendingToolCallsById: [String: OpenHandChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any OpenHandChatTransport) {
    ...
    public var sessionChoices: [OpenHandChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [OpenHandChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: OpenHandPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OpenHandChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OpenHandChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: OpenHandChatMessage) -> OpenHandChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> OpenHandChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return OpenHandChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: OpenHandChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [OpenHandChatMessage],
        incoming: [OpenHandChatMessage]) -> [OpenHandChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [OpenHandChatMessage]) -> [OpenHandChatMessage] {
        var result: [OpenHandChatMessage] = []
    ...
    private static func dedupeKey(for message: OpenHandChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OpenHandChatMessageContent] = [
            OpenHandChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> OpenHandChatAttachmentPayload in
            OpenHandChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> OpenHandChatSessionEntry {
        OpenHandChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: OpenHandChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: OpenHandChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: OpenHandAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OpenHandChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> OpenHandPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
