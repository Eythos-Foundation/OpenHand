...
import Foundation
import Testing
@testable import OpenHandChatUI
...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> OpenHandChatHistoryPayload
{
    OpenHandChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> OpenHandChatSessionEntry {
...
private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [OpenHandChatHistoryPayload],
    sessionsResponses: [OpenHandChatSessionsListResponse] = []) async -> (TestChatTransport, OpenHandChatViewModel)
{
    let transport = TestChatTransport(historyResponses: historyResponses, sessionsResponses: sessionsResponses)
    let vm = await MainActor.run { OpenHandChatViewModel(sessionKey: sessionKey, transport: transport) }
    return (transport, vm)
}

private func loadAndWaitBootstrap(
    vm: OpenHandChatViewModel,
...
private func sendUserMessage(_ vm: OpenHandChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, OpenHandChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OpenHandChatHistoryPayload]
    private let sessionsResponses: [OpenHandChatSessionsListResponse]

    private let stream: AsyncStream<OpenHandChatTransportEvent>
    private let continuation: AsyncStream<OpenHandChatTransportEvent>.Continuation

    init(
        historyResponses: [OpenHandChatHistoryPayload],
        sessionsResponses: [OpenHandChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<OpenHandChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> OpenHandChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OpenHandChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [OpenHandChatAttachmentPayload]) async throws -> OpenHandChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return OpenHandChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OpenHandChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OpenHandChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OpenHandChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = OpenHandChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func stripsInboundMetadataFromHistoryMessages() async throws {
        let history = OpenHandChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [
                AnyCodable([
...
"""]],
        ...
        let vm = await MainActor.run { OpenHandChatViewModel(sessionKey: "main", transport: transport) }
