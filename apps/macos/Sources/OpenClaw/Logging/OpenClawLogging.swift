...
enum OpenHandLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = OpenHandOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OpenHandFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OpenHandLogging.bootstrapIfNeeded()
        let label = OpenHandLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OpenHandOSLogHandler: AppLogLevelBackedHandler {
...
struct OpenHandFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OpenHandLogging.parseLabel(self.label)
