...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.openhand/openhand.json. Prefer OPENHAND_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.openhand/openhand.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.openhand/openhand.json. Restart the gateway to apply."
        }
