...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.openhand.mac`) even if the shipped
    ...
    {
        if environment["OPENHAND_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "openhand.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "openhand.nixMode") { return true }
