...
enum CommandResolver {
    private static let projectRootDefaultsKey = "openhand.gatewayProjectRootPath"
    private static let helperName = "openhand"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let openhandEntry = root.appendingPathComponent("openhand.mjs").path
        if FileManager().isReadableFile(atPath: openhandEntry) { return openhandEntry }
        let binEntry = root.appendingPathComponent("bin/openhand.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/openhand")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let openhandPaths = self.openhandManagedPaths(home: home)
        if !openhandPaths.isEmpty {
            extras.insert(contentsOf: openhandPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + openhandPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func openhandManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".openhand"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func openhandExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOpenHandExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("openhand.mjs").path,
            root.appendingPathComponent("bin/openhand.js").path,
        ]
    ...
    static func hasAnyOpenHandInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.openhandExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func openhandNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let openhandPath = self.projectOpenHandExecutable(projectRoot: root) {
            return [openhandPath, subcommand] + extraArgs
        }
        if let openhandPath = self.openhandExecutable(searchPaths: searchPaths) {
            return [openhandPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "openhand", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            openhand entrypoint missing (looked for dist/index.js or openhand.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real openhand CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/openhand"
        ...
        \(cliSection)
        \(projectSection)
        if command -v openhand >/dev/null 2>&1; then
          CLI="$(command -v openhand)"
          openhand \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/openhand.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/openhand.mjs"
            node "$PRJ/openhand.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/openhand.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/openhand.js"
            node "$PRJ/bin/openhand.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent openhand"
          pnpm --silent openhand \(quotedArgs);
        else
          echo "openhand CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OpenHandConfigFile.loadDict()
