...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["OPENHAND_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/openhand")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("openhand-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("openhand-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/openhand/ (rolling or stdout), or nil if none exist.
