...
enum DebugActions {
    private static let verboseDefaultsKey = "openhand.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "openhand.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = OpenHandPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "OpenHand", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = OpenHandPaths.configURL
        guard
