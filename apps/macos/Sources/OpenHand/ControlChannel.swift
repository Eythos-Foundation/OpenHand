...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OpenHandProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.openhand", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OpenHandKit.AnyCodable]()) {
                $0[$1.key] = OpenHandKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OpenHandProtocol.AnyCodable?) -> [String: OpenHandProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OpenHandProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OpenHandKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OpenHandProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("openhand.control.heartbeat")
    static let controlAgentEvent = Notification.Name("openhand.control.agent")
}
