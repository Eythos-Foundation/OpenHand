...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.openhand", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OpenHandSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OpenHandSessionPreviewEntry,
    ...
    private static func cache(payload: OpenHandSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OpenHandChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OpenHandKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OpenHandKit.AnyCodable]) -> [OpenHandChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(OpenHandChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: OpenHandChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OpenHandChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OpenHandChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OpenHandChatMessage) -> String? {
