...
@Suite struct VoiceWakeRuntimeTests {
    ...
    @Test func hasContentAfterTriggerFalseWhenOnlyTrigger() {
        let triggers = ["openhand"]
        let text = "hey openhand"
    ...
    @Test func gateRequiresGapBetweenTriggerAndCommand() {
        let transcript = "hey openhand do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["openhand"], minPostTriggerGap: 0.3)
