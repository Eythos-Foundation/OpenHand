...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: OpenHandProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OpenHandProtocol.AnyCodable] = [
            "presence": .init([OpenHandProtocol.AnyCodable(entry)]),
        ]
        let payload = OpenHandProtocol.AnyCodable(payloadMap)
