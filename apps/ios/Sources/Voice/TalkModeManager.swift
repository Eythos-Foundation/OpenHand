...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__OPENHAND_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<OpenHandTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.openhand", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = OpenHandTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> OpenHandTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return OpenHandTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> OpenHandTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> OpenHandTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return OpenHandTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> OpenHandTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = OpenHandTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: OpenHandTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: OpenHandAgentEventPayload.self
            ) else {
                continue
            }
