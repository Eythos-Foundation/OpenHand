...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> OpenHandDeviceStatusPayload {
        ...
        return OpenHandDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> OpenHandDeviceInfoPayload {
        ...
        return OpenHandDeviceInfoPayload(
    ...
    private func batteryStatus() -> OpenHandBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: OpenHandBatteryState = switch device.batteryState {
        ...
        return OpenHandBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> OpenHandThermalStatusPayload {
        let state: OpenHandThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return OpenHandThermalStatusPayload(state: state)
    }

    private func storageStatus() -> OpenHandStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return OpenHandStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
