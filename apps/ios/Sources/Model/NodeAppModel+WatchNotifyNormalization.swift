...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: OpenHandWatchNotifyParams) -> OpenHandWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [OpenHandWatchAction]?,
        kind: String?,
        promptId: String?) -> [OpenHandWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> OpenHandWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OpenHandWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                OpenHandWatchAction(id: "approve", label: "Approve"),
                OpenHandWatchAction(id: "decline", label: "Decline", style: "destructive"),
                OpenHandWatchAction(id: "open_phone", label: "Open iPhone"),
                OpenHandWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            OpenHandWatchAction(id: "done", label: "Done"),
            OpenHandWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            OpenHandWatchAction(id: "open_phone", label: "Open iPhone"),
            OpenHandWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: OpenHandWatchRisk?,
        priority: OpenHandNotificationPriority?) -> OpenHandWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: OpenHandNotificationPriority?,
        risk: OpenHandWatchRisk?) -> OpenHandNotificationPriority?
    {
