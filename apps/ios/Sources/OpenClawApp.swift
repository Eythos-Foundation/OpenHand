...
@MainActor
final class OpenHandAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.openhand.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.openhand.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.openhand.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "openhand.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "openhand.watch.promptId"
    static let sessionKeyKey = "openhand.watch.sessionKey"
    static let actionPrimaryIDKey = "openhand.watch.action.primary.id"
    static let actionPrimaryLabelKey = "openhand.watch.action.primary.label"
    static let actionSecondaryIDKey = "openhand.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "openhand.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "openhand.watch.action.primary"
    static let actionSecondaryIdentifier = "openhand.watch.action.secondary"
    static let actionIdentifierPrefix = "openhand.watch.action."
    static let actionIDKeyPrefix = "openhand.watch.action.id."
    static let actionLabelKeyPrefix = "openhand.watch.action.label."
    static let categoryPrefix = "openhand.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: OpenHandWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> OpenHandWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OpenHandWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "OpenHand" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [OpenHandWatchAction]) -> [UNNotificationAction] {
...
struct OpenHandApp: App {
    ...
    @UIApplicationDelegateAdaptor(OpenHandAppDelegate.self) private var appDelegate
...
extension OpenHandApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("OpenHand: installing uncaught exception handler")
