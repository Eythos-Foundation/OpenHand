...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [OpenHandCapability.canvas.rawValue, OpenHandCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(OpenHandCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(OpenHandCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = OpenHandLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(OpenHandCapability.location.rawValue) }

        caps.append(OpenHandCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(OpenHandCapability.watch.rawValue)
        }
        caps.append(OpenHandCapability.photos.rawValue)
        caps.append(OpenHandCapability.contacts.rawValue)
        caps.append(OpenHandCapability.calendar.rawValue)
        caps.append(OpenHandCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(OpenHandCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            OpenHandScreenCommand.record.rawValue,
            OpenHandSystemCommand.notify.rawValue,
            OpenHandChatCommand.push.rawValue,
        ...
        if caps.contains(OpenHandCapability.camera.rawValue) {
            commands.append(OpenHandCameraCommand.list.rawValue)
            commands.append(OpenHandCameraCommand.snap.rawValue)
            commands.append(OpenHandCameraCommand.clip.rawValue)
        }
        if caps.contains(OpenHandCapability.location.rawValue) {
            commands.append(OpenHandLocationCommand.get.rawValue)
        }
        if caps.contains(OpenHandCapability.device.rawValue) {
            commands.append(OpenHandDeviceCommand.status.rawValue)
            commands.append(OpenHandDeviceCommand.info.rawValue)
        }
        if caps.contains(OpenHandCapability.watch.rawValue) {
            commands.append(OpenHandWatchCommand.status.rawValue)
            commands.append(OpenHandWatchCommand.notify.rawValue)
        }
        if caps.contains(OpenHandCapability.photos.rawValue) {
            commands.append(OpenHandPhotosCommand.latest.rawValue)
        }
        if caps.contains(OpenHandCapability.contacts.rawValue) {
            commands.append(OpenHandContactsCommand.search.rawValue)
            commands.append(OpenHandContactsCommand.add.rawValue)
        }
        if caps.contains(OpenHandCapability.calendar.rawValue) {
            commands.append(OpenHandCalendarCommand.events.rawValue)
            commands.append(OpenHandCalendarCommand.add.rawValue)
        }
        if caps.contains(OpenHandCapability.reminders.rawValue) {
            commands.append(OpenHandRemindersCommand.list.rawValue)
            commands.append(OpenHandRemindersCommand.add.rawValue)
        }
        if caps.contains(OpenHandCapability.motion.rawValue) {
            commands.append(OpenHandMotionCommand.activity.rawValue)
            commands.append(OpenHandMotionCommand.pedometer.rawValue)
        }
