...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OpenHandContactsSearchParams) async throws -> OpenHandContactsSearchPayload {
        ...
        return OpenHandContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OpenHandContactsAddParams) async throws -> OpenHandContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OpenHandContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OpenHandContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OpenHandContactPayload {
        OpenHandContactPayload(
