...
final class MotionService: MotionServicing {
    func activities(params: OpenHandMotionActivityParams) async throws -> OpenHandMotionActivityPayload {
        ...
        let mapped: [OpenHandMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return OpenHandMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: OpenHandPedometerParams) async throws -> OpenHandPedometerPayload {
        ...
        let payload: OpenHandPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = OpenHandPedometerPayload(
