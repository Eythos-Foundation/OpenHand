...
struct IOSGatewayChatTransport: OpenHandChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.openhand", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> OpenHandChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OpenHandChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OpenHandChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OpenHandChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OpenHandChatAttachmentPayload]) async throws -> OpenHandChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [OpenHandChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(OpenHandChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(OpenHandGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenHandGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: OpenHandChatEventPayload.self)
                        {
