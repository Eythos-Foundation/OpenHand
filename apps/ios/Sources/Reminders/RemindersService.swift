...
final class RemindersService: RemindersServicing {
    func list(params: OpenHandRemindersListParams) async throws -> OpenHandRemindersListPayload {
        ...
        let payload: [OpenHandReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return OpenHandReminderPayload(
        ...
        return OpenHandRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OpenHandRemindersAddParams) async throws -> OpenHandRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OpenHandReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OpenHandRemindersAddPayload(reminder: payload)
    }
