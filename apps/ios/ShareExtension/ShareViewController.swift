...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.openhand.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to OpenHand", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to OpenHand gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to OpenHand.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "OpenHandShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "OpenHand is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "OpenHand Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("openhand-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
