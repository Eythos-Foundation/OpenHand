...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `openhand-e2e-${name}-`));
  const configDir = path.join(homeDir, ".openhand");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          OPENHAND_SKIP_PROVIDERS: "1",
          OPENHAND_SKIP_GMAIL_WATCHER: "1",
          OPENHAND_SKIP_CRON: "1",
          OPENHAND_SKIP_BROWSER_CONTROL_SERVER: "1",
          OPENHAND_SKIP_CANVAS_HOST: "1",
          OPENHAND_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },
