...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked OPENHAND_HOME.
  delete process.env.OPENHAND_HOME;
  process.env.OPENHAND_STATE_DIR = path.join(base, ".openhand");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "openhand-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".openhand", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
