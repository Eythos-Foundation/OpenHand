...
read_when:
  ...
  - Wiring external systems into OpenHand
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your OpenHand config directory (typically `~/.openhand/hooks/transforms`).
...
- `openhand webhooks gmail setup` writes `hooks.gmail` config for `openhand webhooks gmail run`.
