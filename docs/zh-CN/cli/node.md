...
summary: "`openhand node` 的 CLI 参考（无头节点主机）"
title: node
x-i18n:
...
```bash
openhand node install --host <gateway-host> --port 18789
```
...
```bash
openhand node status
openhand node stop
openhand node restart
openhand node uninstall
```

使用 `openhand node run` 运行前台节点主机（无服务）。
...
- `openhand approvals --node <id|name|ip>`（从 Gateway 网关编辑）
