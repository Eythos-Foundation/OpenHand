...
你可以在 `~/.openhand/openhand.json` 中覆盖此设置：
...
所有日志配置都在 `~/.openhand/openhand.json` 的 `logging` 下。
...
- OpenHand 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `openhand plugins enable diagnostics-otel` 启用插件。
...
- `openhand.tokens`（计数器，属性：`openhand.token`、`openhand.channel`、`openhand.provider`、`openhand.model`）
- `openhand.cost.usd`（计数器，属性：`openhand.channel`、`openhand.provider`、`openhand.model`）
- `openhand.run.duration_ms`（直方图，属性：`openhand.channel`、`openhand.provider`、`openhand.model`）
- `openhand.context.tokens`（直方图，属性：`openhand.context`、`openhand.channel`、`openhand.provider`、`openhand.model`）

消息流：

- `openhand.webhook.received`（计数器，属性：`openhand.channel`、`openhand.webhook`）
- `openhand.webhook.error`（计数器，属性：`openhand.channel`、`openhand.webhook`）
- `openhand.webhook.duration_ms`（直方图，属性：`openhand.channel`、`openhand.webhook`）
- `openhand.message.queued`（计数器，属性：`openhand.channel`、`openhand.source`）
- `openhand.message.processed`（计数器，属性：`openhand.channel`、`openhand.outcome`）
- `openhand.message.duration_ms`（直方图，属性：`openhand.channel`、`openhand.outcome`）

队列 + 会话：

- `openhand.queue.lane.enqueue`（计数器，属性：`openhand.lane`）
- `openhand.queue.lane.dequeue`（计数器，属性：`openhand.lane`）
- `openhand.queue.depth`（直方图，属性：`openhand.lane` 或 `openhand.channel=heartbeat`）
- `openhand.queue.wait_ms`（直方图，属性：`openhand.lane`）
- `openhand.session.state`（计数器，属性：`openhand.state`、`openhand.reason`）
- `openhand.session.stuck`（计数器，属性：`openhand.state`）
- `openhand.session.stuck_age_ms`（直方图，属性：`openhand.state`）
- `openhand.run.attempt`（计数器，属性：`openhand.attempt`）
...
- `openhand.webhook.error`
  - `openhand.channel`、`openhand.webhook`、`openhand.chatId`、`openhand.error`
- `openhand.message.processed`
  - `openhand.channel`、`openhand.outcome`、`openhand.chatId`、`openhand.messageId`、`openhand.sessionKey`、`openhand.sessionId`、`openhand.reason`
- `openhand.session.stuck`
  - `openhand.state`、`openhand.ageMs`、`openhand.queueDepth`、`openhand.sessionKey`、`openhand.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `openhand doctor`。
