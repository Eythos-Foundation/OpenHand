...
Deploy OpenHand on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/openhand` — Control UI
...
- `OPENHAND_STATE_DIR=/data/.openhand` (recommended)
- `OPENHAND_WORKSPACE_DIR=/data/workspace` (recommended)
- `OPENHAND_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your OpenHand state + workspace so you can migrate to another host without losing config or memory.
