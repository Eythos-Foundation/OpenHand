...
Deploy OpenHand on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/Eythos-Foundation/OpenHand)
...
repository configures everything needed to run OpenHand:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: OPENHAND_STATE_DIR
        value: /data/.openhand
      - key: OPENHAND_WORKSPACE_DIR
        value: /data/workspace
      - key: OPENHAND_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/openhand`.
...
If you use the original OpenHand repository, Render will not auto-deploy your OpenHand. To update it, run a manual Blueprint sync from the dashboard.
...
For OpenHand, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any OpenHand host.
