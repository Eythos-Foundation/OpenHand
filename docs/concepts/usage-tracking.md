...
- `/usage cost` in chats: local cost summary aggregated from OpenHand session logs.
- CLI: `openhand status --usage` prints a full per-provider breakdown.
- CLI: `openhand channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
