...
```bash
openhand plugins install @openhand/msteams
```

Local checkout (when running from a git repo):

```bash
openhand plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
OpenHand will offer the local install path automatically.
...
3. Configure OpenHand with those credentials.
...
- Talk to OpenHand via Teams DMs, group chats, or channels.
...
- On startup, OpenHand resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.openhand/openhand.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring OpenHand, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `openhand-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `openhand plugins install @openhand/msteams`
   - From a local checkout: `openhand plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "OpenHand in Teams", "full": "OpenHand in Teams" },
...
OpenHand handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, OpenHand only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/OpenHandShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

OpenHand sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `openhand message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.openhand/msteams-polls.json`.
...
```bash
openhand message send --channel msteams \
...
openhand message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
openhand message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
openhand message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
openhand message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
