...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects OpenHand to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
openhand plugins install @openhand/feishu
```

Local checkout (when running from a git repo):

```bash
openhand plugins install ./extensions/feishu
```
...
If you just installed OpenHand, run the wizard:
...
2. Configuring app credentials in OpenHand
...
1. You already ran `openhand channels add` for Feishu
2. The gateway is running (`openhand gateway status`)
...
```bash
openhand pairing approve feishu <CODE>
```
...
2. Run `openhand logs --follow` and look for `chat_id`
...
2. Run `openhand logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `openhand gateway status`  | Show gateway status           |
| `openhand gateway install` | Install/start gateway service |
| `openhand gateway stop`    | Stop gateway service          |
| `openhand gateway restart` | Restart gateway service       |
| `openhand logs --follow`   | Tail gateway logs             |
...
4. Check logs: `openhand logs --follow`
...
5. Ensure the gateway is running: `openhand gateway status`
6. Check logs: `openhand logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.openhand/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.openhand/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
