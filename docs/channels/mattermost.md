---
summary: "Mattermost bot setup and OpenHand config"
read_when:
...
```bash
openhand plugins install @openhand/mattermost
```

Local checkout (when running from a git repo):

```bash
openhand plugins install ./extensions/mattermost
```
...
4. Configure OpenHand and start the gateway.
...
- Approve via:
  - `openhand pairing list mattermost`
  - `openhand pairing approve mattermost <CODE>`
...
Use these target formats with `openhand message send` or cron/webhooks:
