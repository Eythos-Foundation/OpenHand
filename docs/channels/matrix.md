...
Matrix is an open, decentralized messaging protocol. OpenHand connects as a Matrix **user**
...
```bash
openhand plugins install @openhand/matrix
```

Local checkout (when running from a git repo):

```bash
openhand plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `openhand plugins install @openhand/matrix`
   - From a local checkout: `openhand plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: OpenHand calls the same
     login endpoint, stores the access token in `~/.openhand/credentials/matrix/credentials.json`,
...
- On first connection, OpenHand requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  OpenHand logs a warning.
...
`~/.openhand/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `openhand pairing list matrix`
  - `openhand pairing approve matrix <CODE>`
...
- On startup, OpenHand resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
