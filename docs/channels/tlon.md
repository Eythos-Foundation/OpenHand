...
Tlon is a decentralized messenger built on Urbit. OpenHand connects to your Urbit ship and can
...
```bash
openhand plugins install @openhand/tlon
```

Local checkout (when running from a git repo):

```bash
openhand plugins install ./extensions/tlon
```
...
By default, OpenHand blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `openhand message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, OpenHand replies in-thread.
