...
```bash
openhand plugins install @openhand/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
openhand plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "OpenHand" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openhand pairing list nextcloud-talk`
  - `openhand pairing approve nextcloud-talk <CODE>`
