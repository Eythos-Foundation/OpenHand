...
read_when:
  - Setting up Twitch chat integration for OpenHand
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. OpenHand connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openhand plugins install @openhand/twitch
```

Local checkout (when running from a git repo):

```bash
openhand plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `OPENHAND_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openhand", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use OPENHAND_TWITCH_ACCESS_TOKEN env var)
...
```bash
OPENHAND_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
