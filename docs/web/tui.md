...
```bash
openhand tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `openhand tui --deliver`
...
- Local shell commands receive `OPENHAND_SHELL=tui-local` in their environment.
...
- Check the Gateway logs: `openhand logs --follow`.
- Confirm the agent can run: `openhand status` and `openhand models status`.
...
- No agents in picker: check `openhand agents list` and your routing config.
