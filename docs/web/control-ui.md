...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/openhand`)
...
If the page fails to load, start the Gateway first: `openhand gateway`.
...
you revoke it with `openhand devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.openhand/openhand.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop openhand`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. OpenHand
...
OpenHand **blocks** Control UI connections without device identity.
