...
This plan defines how OpenHand should support ACP coding agents in thread-capable channels (Discord first) with production-level lifecycle and recovery.
...
- OpenHand core owns ACP control plane concerns
...
OpenHand should not reimplement ACP transport internals in core.
OpenHand should not rely on a pure plugin-only interception path for routing.

## North-star architecture (holy grail)

Treat ACP as a first-class control plane in OpenHand, with pluggable runtime adapters.
...
- move ACP control-plane state to a dedicated SQLite store (WAL mode) under OpenHand state dir
...
- ACP event stream is normalized to OpenHand reply chunks
...
For the first production backend (`extensions/acpx`), OpenHand and acpx are
...
- encoded payload fields:
  - `name` (acpx named session; uses OpenHand `sessionKey`)
...
- OpenHand consumes ndjson events from `acpx --format json --json-strict`
