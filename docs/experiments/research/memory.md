...
read_when:
  - Designing workspace memory (~/.openhand/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep OpenHand integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.openhand/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.openhand/workspace` as canonical human-readable memory.
...
```
~/.openhand/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in OpenHand**, but keep a separable core library.

### Why integrate into OpenHand?

- OpenHand already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `openhand memory recall "…" --k 25 --since 30d`
  - `openhand memory reflect --since 7d`
...
Pragmatic take for `~/.openhand/workspace`:
