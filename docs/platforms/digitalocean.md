---
summary: "OpenHand on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up OpenHand on DigitalOcean
  - Looking for cheap VPS hosting for OpenHand
title: "DigitalOcean"
---

# OpenHand on DigitalOcean

## Goal

Run a persistent OpenHand Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status openhand-gateway.service

# View logs
journalctl --user -u openhand-gateway.service -f
```
...
```bash
openhand channels login whatsapp
# Scan QR code
```
...
- `~/.openhand/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u openhand --no-pager -n 50
```
