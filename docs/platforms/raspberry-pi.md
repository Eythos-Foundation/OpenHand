---
summary: "OpenHand on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up OpenHand on a Raspberry Pi
  - Running OpenHand on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# OpenHand on Raspberry Pi

## Goal

Run a persistent, always-on OpenHand Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
openhand config set gateway.bind tailnet
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/openhand-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
export NODE_COMPILE_CACHE=/var/tmp/openhand-compile-cache
mkdir -p /var/tmp/openhand-compile-cache
export OPENHAND_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `OPENHAND_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running OpenHand, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit openhand
```

```ini
[Service]
Environment=OPENHAND_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/openhand-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep OpenHand state/cache on SSD-backed storage to avoid SD-card
...
Most OpenHand features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled openhand
...
```bash
# Check logs
journalctl -u openhand --no-pager -n 100

# Common fix: rebuild
cd ~/openhand  # if using hackable install
npm run build
sudo systemctl restart openhand
```
