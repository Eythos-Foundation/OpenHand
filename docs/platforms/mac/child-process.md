...
service via the external `openhand` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.openhand.gateway`
  (or `ai.openhand.<profile>` when using `--profile`/`OPENHAND_PROFILE`; legacy `com.openhand.*` is supported).
