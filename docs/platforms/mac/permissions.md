...
- Same path: run the app from a fixed location (for OpenHand, `dist/OpenHand.app`).
...
```bash
sudo tccutil reset Accessibility ai.openhand.mac
sudo tccutil reset ScreenCapture ai.openhand.mac
...
Workaround: move files into the OpenHand workspace (`~/.openhand/workspace`) if you want to avoid per-folder grants.
