...
- With Anthropic API-key auth profiles, OpenHand seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, OpenHand injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
OpenHand exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.openhand/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$OPENHAND_STATE_DIR/logs/cache-trace.jsonl`
...
- `OPENHAND_CACHE_TRACE=1` enables cache tracing.
- `OPENHAND_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `OPENHAND_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `OPENHAND_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `OPENHAND_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
