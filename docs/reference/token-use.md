---
summary: "How OpenHand builds prompt context and reports token usage + costs"
read_when:
...
OpenHand tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

OpenHand assembles its own system prompt on every run. It includes:
...
For images, OpenHand downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from OpenHand session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `openhand status --usage` and `openhand channels list` show
...
`cacheWrite`. If pricing is missing, OpenHand shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. OpenHand can
...
Anthropic's 1M context window is currently beta-gated. OpenHand can inject the
...
OpenHand skips the `context-1m-*` beta header because Anthropic currently
