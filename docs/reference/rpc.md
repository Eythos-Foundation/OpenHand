...
OpenHand integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- OpenHand owns lifecycle when `channels.signal.autoStart=true`.
...
- OpenHand spawns `imsg rpc` as a child process (legacy iMessage integration).
