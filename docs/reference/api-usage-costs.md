...
OpenHand features that can generate provider usage or paid API calls.
...
- `openhand status --usage` and `openhand channels list` show provider **usage windows**
...
OpenHand can pick up credentials from:
...
`openhand models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
