...
read_when:
  - You want to place an outbound voice call from OpenHand
...
Voice calls for OpenHand via a plugin. Supports outbound notifications and
...
- Use `openhand voicecall ...` or the `voice_call` tool
...
```bash
openhand plugins install ./extensions/voice-call
