...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in OpenHand
...
Running OpenHand via Node with `tsx` fails at startup with:

```
[openhand] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `openhand status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch openhand.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node openhand.mjs status` works on Node 25.
