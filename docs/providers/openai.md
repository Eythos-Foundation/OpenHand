---
summary: "Use OpenAI via API keys or Codex subscription in OpenHand"
read_when:
  - You want to use OpenAI models in OpenHand
...
OpenAI explicitly supports subscription OAuth usage in external tools/workflows like OpenHand.
...
```bash
openhand onboard --auth-choice openai-api-key
# or non-interactive
openhand onboard --openai-api-key "$OPENAI_API_KEY"
```
...
openhand onboard --auth-choice openai-codex

# Or run OAuth directly
openhand models auth login --provider openai-codex
```
...
OpenHand uses `pi-ai` for model streaming. For both `openai/*` and
...
For `openai/*` (Responses API), OpenHand also enables WebSocket warm-up by
...
OpenAI docs describe warm-up as optional. OpenHand enables it by default for
...
`baseUrl` on `api.openai.com`), OpenHand now auto-enables OpenAI server-side
