---
summary: "Use MiniMax M2.5 in OpenHand"
read_when:
  - You want MiniMax models in OpenHand
...
- **Compatibility:** OpenHand still accepts legacy `MiniMax-M2.5-Lightning` configs, but prefer
...
```bash
openhand plugins enable minimax-portal-auth  # skip if already loaded.
openhand gateway restart  # restart if gateway is already running
openhand onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/openhand/openhand/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `openhand.json`:
...
- Use `openhand models list` and `openhand models set minimax/MiniMax-M2.5` to switch.
...
- Running `openhand configure` and selecting **MiniMax M2.5**, or
