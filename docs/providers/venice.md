---
summary: "Use Venice AI privacy-focused models in OpenHand"
read_when:
  - You want privacy-focused inference in OpenHand
...
```bash
openhand onboard --auth-choice venice-api-key
```
...
```bash
openhand agent --model venice/llama-3.3-70b --message "Hello, are you working?"
```

## Model Selection

After setup, OpenHand shows all available Venice models. Pick based on your needs:
...
```bash
openhand models set venice/claude-opus-45
openhand models set venice/llama-3.3-70b
```

List all available models:

```bash
openhand models list | grep venice
```

## Configure via `openhand configure`

1. Run `openhand configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
OpenHand automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
openhand agent --model venice/llama-3.3-70b --message "Quick health check"

# Use Claude via Venice (anonymized)
openhand agent --model venice/claude-opus-45 --message "Summarize this task"

# Use uncensored model
openhand agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
openhand agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
openhand agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `openhand models list` to see currently available models. Some models may be temporarily offline.
