...
- Publish as `openhand/plugin-sdk` (or export from core under `openhand/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `OpenHandPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: OpenHandConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: OpenHandConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: OpenHandConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: OpenHandConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: OpenHandConfig): string;
  };
};
```
...
- Introduce `openhand/plugin-sdk`.
- Add `api.runtime` to `OpenHandPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `openhandRuntime: ">=2026.2.0"`).
