...
- During heartbeat runs, OpenHand treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openhand sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, OpenHand skips the heartbeat run entirely (no model call).
...
```bash
openhand system event --text "Check for urgent follow-ups" --mode now
```
