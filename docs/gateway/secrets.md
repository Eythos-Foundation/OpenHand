...
OpenHand supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
When onboarding runs in interactive mode and you choose SecretRef storage, OpenHand runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). OpenHand validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/OpenHand QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/openhand"],
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `openhand.json` refs)
...
When reload-time activation fails after a healthy state, OpenHand enters degraded secrets state.
...
Credential-sensitive command paths that opt in (for example `openhand memory` remote-memory paths and `openhand qr --remote`) can resolve supported SecretRefs via gateway snapshot RPC.
...
- Snapshot refresh after backend secret rotation is handled by `openhand secrets reload`.
...
```bash
openhand secrets audit --check
openhand secrets configure
openhand secrets audit --check
```
...
- plaintext values at rest (`openhand.json`, `auth-profiles.json`, `.env`)
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `openhand.json` refs)
...
- lets you select supported secret-bearing fields in `openhand.json` plus `auth-profiles.json` for one agent scope
...
- `openhand secrets configure --providers-only`
- `openhand secrets configure --skip-provider-setup`
- `openhand secrets configure --agent <id>`
...
```bash
openhand secrets apply --from /tmp/openhand-secrets-plan.json
openhand secrets apply --from /tmp/openhand-secrets-plan.json --dry-run
```
...
OpenHand intentionally does not write rollback backups containing historical plaintext secret values.
