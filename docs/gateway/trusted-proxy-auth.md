...
read_when:
  - Running OpenHand behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of OpenHand
...
- You run OpenHand behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. OpenHand checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. OpenHand extracts the user identity from the configured header
...
- OpenHand can stay on loopback HTTP behind the proxy.
...
If OpenHand itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://openhand.example.com
    to: http://openhand-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
openhand.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy openhand:18789 {
...
location / {
    ...
    proxy_pass http://openhand:18789;
...
`openhand security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update OpenHand config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `openhand security audit` and review findings
