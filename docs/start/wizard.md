...
The onboarding wizard is the **recommended** way to set up OpenHand on macOS,
...
`openhand dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
(`web_fetch` works without a key). Easiest path: `openhand configure --section web`
...
2. **Workspace** — Location for agent files (default `~/.openhand/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, the wizard asks you to run `openhand doctor` first.
</Note>
...
Use `openhand agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.openhand/workspace-<agentId>`.
...
- CLI command reference: [`openhand onboard`](/cli/onboard)
