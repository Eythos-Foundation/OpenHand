...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openhand sandbox explain`.
...
Legacy `agent.*` configs are migrated by `openhand doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=openhand-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENHAND_STATE_DIR:-$HOME/.openhand}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
