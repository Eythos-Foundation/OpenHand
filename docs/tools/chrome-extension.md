---
summary: "Chrome extension: let OpenHand drive your existing Chrome tab"
read_when:
...
The OpenHand Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate openhand-managed Chrome profile.
...
OpenHand then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the OpenHand release (npm package) as static files. There is no separate “build” step.

After upgrading OpenHand:

- Re-run `openhand browser extension install` to refresh the installed files under your OpenHand state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

OpenHand ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `OPENHAND_GATEWAY_TOKEN`)
...
- Open the tab you want OpenHand to control.
...
- `ON`: attached; OpenHand can drive that tab.
...
Debugging: `openhand sandbox explain`
...
`openhand browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `openhand browser extension install` first to copy the extension to a stable location under your OpenHand state directory.
...
- **This is not isolated** like the dedicated openhand-managed profile.
