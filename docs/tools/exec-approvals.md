...
- **gateway host** → `openhand` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.openhand/exec-approvals.sock",
...
- allowlist entries live in host-local `~/.openhand/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `openhand approvals allowlist ...`).
- `openhand security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `openhand doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
`~/.openhand/exec-approvals.json` directly.

CLI: `openhand approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
