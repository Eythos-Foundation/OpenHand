---
summary: "CLI reference for `openhand onboard` (interactive onboarding wizard)"
read_when:
...
```bash
openhand onboard
openhand onboard --flow quickstart
openhand onboard --flow manual
openhand onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`OPENHAND_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- Fastest first chat: `openhand dashboard` (Control UI, no channel setup).
