...
Read-only diff viewer plugin for **OpenHand** agents.
...
Set plugin-wide defaults in `~/.openhand/openhand.json`:
...
Before:
OpenHand supports plugins.

After:
OpenHand supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/openhand-diffs`).
