# Google Gemini CLI Auth (OpenHand plugin)
...
```bash
openhand plugins enable google-gemini-cli-auth
```
...
```bash
openhand models auth login --provider google-gemini-cli --set-default
```
...
- `OPENHAND_GEMINI_OAUTH_CLIENT_ID` / `GEMINI_CLI_OAUTH_CLIENT_ID`
- `OPENHAND_GEMINI_OAUTH_CLIENT_SECRET` / `GEMINI_CLI_OAUTH_CLIENT_SECRET`
