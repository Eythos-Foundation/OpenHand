...
metadata: { "openhand": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == OpenHand `sessions_spawn`
- **File I/O** == OpenHand `read`/`write`
- **Remote fetch** == OpenHand `web_fetch` (or `exec` with curl when POST is required)
