...
import type { OpenHandPluginApi, OpenHandPluginService } from "openhand/plugin-sdk";
...
function normalizeDenyList(cfg: OpenHandPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: OpenHandPluginApi["config"]): string[] {
...
export default function register(api: OpenHandPluginApi) {
  ...
  const timerService: OpenHandPluginService = {
