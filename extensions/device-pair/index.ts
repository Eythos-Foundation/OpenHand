...
function resolveGatewayPort(cfg: OpenHandPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.OPENHAND_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: OpenHandPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: OpenHandPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the OpenHand iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the OpenHand iOS app:",
            "",
            "```",
            qrAscii,
            "```",
