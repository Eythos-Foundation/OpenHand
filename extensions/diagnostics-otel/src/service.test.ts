...
vi.mock("openhand/plugin-sdk", async () => {
  const actual = await vi.importActual<typeof import("openhand/plugin-sdk")>("openhand/plugin-sdk");
  return {
    ...actual,
...
import type { OpenHandPluginServiceContext } from "openhand/plugin-sdk";
import { emitDiagnosticEvent } from "openhand/plugin-sdk";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/openhand-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): OpenHandPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("openhand.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("openhand.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("openhand.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("openhand.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("openhand.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("openhand.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("openhand.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("openhand.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("openhand.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("openhand.webhook.processed");
    expect(spanNames).toContain("openhand.message.processed");
    expect(spanNames).toContain("openhand.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["openhand.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("openhand.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "openhand.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["openhand.reason"]).toBe("string");
    expect(String(attrs?.["openhand.reason"])).not.toContain(
