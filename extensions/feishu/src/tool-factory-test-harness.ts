import type { AnyAgentTool, OpenHandPluginApi } from "openhand/plugin-sdk";
...
export function createToolFactoryHarness(cfg: OpenHandPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<OpenHandPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
