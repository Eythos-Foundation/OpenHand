...
`openhand.invoke --each`) without adding new OpenHand code per workflow.
...
This extension depends on OpenHand internal modules (the embedded agent runner).
It is intended to ship as a **bundled** OpenHand extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.openhand/extensions` as a standalone plugin directory.
