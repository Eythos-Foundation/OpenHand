...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["openhandPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["openhand", "pollId"]) ??
    readNestedString(value, ["openhand", "poll", "id"]) ??
    readNestedString(value, ["data", "openhandPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "openhand", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          openhandPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "openhand poll vote",
          displayText: "Vote recorded",
          value: { openhandPollId: pollId, pollId },
        },
      },
    ],
  };
