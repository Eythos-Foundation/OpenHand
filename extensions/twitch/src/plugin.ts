/**
 * Twitch channel plugin for OpenHand.
 *
...
import { buildChannelConfigSchema } from "openhand/plugin-sdk";
 ...
 * for OpenHand. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: OpenHandConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: OpenHandConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: OpenHandConfig): boolean => {
