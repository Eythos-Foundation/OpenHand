...
Official Voice Call plugin for **OpenHand**.
...
Docs: `https://docs.openhand.ai/plugins/voice-call`
Plugin system: `https://docs.openhand.ai/plugin`

## Install (local dev)

### Option A: install via OpenHand (recommended)
...
```bash
mkdir -p ~/.openhand/extensions
cp -R extensions/voice-call ~/.openhand/extensions/voice-call
cd ~/.openhand/extensions/voice-call && pnpm install
```
...
```bash
openhand voicecall call --to "+15555550123" --message "Hello from OpenHand"
openhand voicecall continue --call-id <id> --message "Any questions?"
openhand voicecall speak --call-id <id> --message "One moment"
openhand voicecall end --call-id <id>
openhand voicecall status --call-id <id>
openhand voicecall tail
