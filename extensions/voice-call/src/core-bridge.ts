...
function resolveOpenHandRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.OPENHAND_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set OPENHAND_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveOpenHandRoot(), "dist", "extensionAPI.js");
