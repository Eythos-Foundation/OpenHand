...
export async function handleWhatsAppStart(host: OpenHandApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: OpenHandApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: OpenHandApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: OpenHandApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: OpenHandApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: OpenHandApp): string {
...
function resolveGatewayHttpAuthHeader(host: OpenHandApp): string | null {
...
function buildGatewayHttpHeaders(host: OpenHandApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: OpenHandApp) {
...
export function handleNostrProfileToggleAdvanced(host: OpenHandApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: OpenHandApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: OpenHandApp) {
  const state = host.nostrProfileFormState;
