...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/openhand/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/openhand");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/openhand/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENHAND_CONTROL_UI_BASE_PATH__ = "/openhand";
    const app = mountApp("/openhand/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/openhand");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/openhand/sessions");
  });
